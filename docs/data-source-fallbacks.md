...
- คำ `วัดได้` / `measured` / `ราคาเฉลี่ย ÷` เข้า `PE_LABEL_SKIP` **ตั้งแต่ PR #56 (W1)** (งานพี่น้อง ยังไม่ merge ณ 22 ก.ย. 69) — ก่อนหน้านั้นโค้ดข้ามเฉพาะคำในรายการข้างบน จึงควรตั้งป้ายด้วยคำที่อยู่ในรายการนั้นด้วย

---

## 8. หมายเหตุคิว

ถ้า `npm run queue -- preflight` พิมพ์บรรทัด `⚠ statement unknown: N (fetch-failed M · no-ua K)` (บรรทัดนี้มี **ตั้งแต่ PR #57 (W11)** — งานพี่น้อง ยังไม่ merge และยังไม่มีในโค้ดที่ทดสอบ 22 ก.ย. 69) ⇒ **รัน preflight ซ้ำก่อนเริ่มคิว** — การดึงล้ม (`fetch-failed`) ไม่ได้แปลว่างบเปลี่ยน แต่ถ้าไม่ลองใหม่ แถวเหล่านั้นจะถูก escalate เงียบ ๆ (fetch-failed เป็น bucket PLUMBING ใน `tools/queue/triage.js` — ไม่ควรกลายเป็นงานวิเคราะห์เพราะเน็ตสะดุด) · ยังล้มซ้ำ = แจ้ง controller พร้อมรายชื่อ symbol
