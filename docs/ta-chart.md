...
| C10 | gate/cron เดิม | ✅ ผ่าน | `npm run verify` เขียวครบ (8 ขั้น ณ ตอนนั้น — ปัจจุบัน 14) (check-reports 0 error 1 warning ไม่เกี่ยว TA + build + build-test + engine-exec 784/784 + skeleton-test + check-site + `test:ohlc` + `test:ta`) exit 0 · `git status --porcelain reports/` ว่างเปล่า (ไม่มีไฟล์ใน `reports/` ถูกแตะ) · E35/E36/E37 อ่านจาก `report-data`/header ใน source ซึ่ง TA ไม่แตะเลย |
