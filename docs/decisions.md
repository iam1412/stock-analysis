...
- **preflight**: ถอด `!e.v3` (คิวอายุนับใบ v3) · แถว v3 = บรรทัดตาม bucket (PREPATCH → pre-patch มือแล้ว `ship` · LIGHT/FULL → หลัง `ship --prepatch` pre-patch มือถ้ายังไม่สด แล้ว `prep`) · **pre-patch อัตโนมัติของใบ v3 ยังเป็น 4b** (parseGateFailures regex `.html` · revert `git checkout -- reports/<sym>.html` · `ship.js:243` existsSync ทิ้ง .html ที่ลบ) — open-items #66
