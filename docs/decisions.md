...
- **R9 `verify:cron` + `check-v3` (ปิด #61)** — queue-test `0b` ตรึงจำนวนขั้นของ cron ไว้ (5 → 6 · ตรวจพบตอนซ้อมโค้ดในแผน ไม่ได้อยู่ใน findings) · gen-docs แก้ marker 3 ไฟล์ (CLAUDE.md §8 · README · price-refresh) · วัดจริง: `verify:cron` ~8 วินาที (`check-v3` 0.15 วินาที)
...
- **token log `v3-lane:`** แทน `v3-skipped: N` ของ P4 (บรรทัดนั้นไม่มีแล้ว) — ใช้ตรวจเกณฑ์ (f) 3 รอบสะอาดหลัง merge · รูปจริง (fix round 1 M3 เพิ่มถัง "ล้ม" ให้ผลรวม = N):
  `ℹ v3-lane: N ใบ · อัปเดต W · ไม่เปลี่ยน U · freeze F · ล้ม X · ข้ามเพราะตลาดเปิด I · ข้าม not-on-exchange D` — N = W+U+F+X+I+D · ล้ม = fetch-failed / JSON ใบ v3 อ่านไม่ได้ / `patch-failed` · freeze = `decide()` (drift/split/flip/currency) / `bad-chart` / `patch-rejected`
...
- **ข้อรองจาก review ที่ยกไป P6** — (1) คืนที่ quote ค้าง ใบนั้นยังนับเป็น evaluated ⇒ flag ที่ค้างอยู่ถูกล้างคืนนั้น (ยกใหม่เมื่อได้ quote สด พร้อม `flaggedAt` ใหม่) · (2) `chartFor` รันก่อน `decide()` ⇒ แถวที่ freeze เสีย fetch รายสัปดาห์เพิ่ม 1 ครั้ง · (3) `v3Lines` พิมพ์คำสั่ง `--force` ให้ทุกแถว v3 ไม่แยก bucket (รวม not-on-exchange — P6 ทำให้รู้ bucket)
- **จำนวนเทสที่ปิดงาน** — market 27 · io 31 · cron 63 · sidecar 57 · scanners-cron 21 · queue-test 559 · update-prices-test 520 · `verify:cron` มี `check-v3` (จำนวนขั้นอยู่ใน marker ของ CLAUDE.md §8 — ห้ามเขียนเลขนอก marker)
