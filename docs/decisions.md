...
- **คำตัดสิน controller ระหว่างทำ (ledger 36 rulings + 5 carries · SDD workspace) — ที่เปลี่ยนพฤติกรรม**:
  - **v2 NEW `prep` ช่วง 2b ยิงเน็ตเพิ่ม 2 ครั้ง (4 ครั้งถ้ามี `--brand`)** เพื่อเขียน sidecar · `--json` ตัวใดล้ม = throw **ก่อน**เขียน `.md` (fail-loud ตั้งใจ — ไม่มี sidecar = init รันไม่ได้) · `buildSidecar` throw เมื่อ `errors.fin`/`errors.stats` หรือสกุลเงิน quote ≠ งบ · error แหล่งอื่นผ่านเป็น `sourceErrors` (นอก `market`) ให้ init พิมพ์เตือน 1 บรรทัด · prep โหมด UPDATE byte-identical เท่าเดิม
