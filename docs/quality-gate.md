...
มี gate หลายชั้น ต้องผ่านทั้งหมด **ก่อน push เสมอ** (มี `pre-push` hook บังคับซ้ำ <!-- gen:verify-steps -->17<!-- /gen:verify-steps --> ขั้น):

```bash
npm run verify           # ★ ครบชุด <!-- gen:verify-steps -->17<!-- /gen:verify-steps --> ขั้น: <!-- gen:verify-chain-plain -->update-prices-test → dead-ticker-test → tag-apply-test → queue-test → docs-test → prep-stock-test → tags-test → report-values-test → check-reports → self-test → ohlc-test → ta-engine-test → build → build-test → engine-exec → skeleton-test → check-site<!-- /gen:verify-chain-plain -->
