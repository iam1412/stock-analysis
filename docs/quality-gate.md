...
| W06 | warn | สรุป "ส่วนต่างจากราคา" ตรงกับ MOS | ช่องต้องเป็น `MOS ~ ±X%` (คลังคำคงที่ · cron เขียน) และ X = ตัวเลขเดียวกับ `.big` ตามที่พิมพ์ (อ่านจาก `.big` ตรง ๆ ไม่คำนวณจาก FV) |
