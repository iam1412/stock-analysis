...
git commit -m "price: refresh N symbols (YYYY-MM-DD)" && push
```
...
- **เคลียร์คิว:** เปิด session สั่ง "เคลียร์คิว price-flags" → อ่าน `price-flags.json` → re-analysis ตาม bulk workflow (§3) ทุกกติกาเดิม (ตัว suspect-split เข้าข่าย "หุ้นยาก" → escalate Opus) · ปล่อยค้าง = วันที่ราคาเก่าลงจนโดน staleness gate เดิม (warn 45 / error 120 วัน) กดดันตามปกติ
