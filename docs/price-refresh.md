...
GitHub Actions (`.github/workflows/update-prices.yml`) config ตั้ง **21:15 UTC ทุกวัน** (= 04:15 น. ไทยวันถัดไป — เดิม 07:17 น. ไทย / 00:17 UTC ก่อนแก้ open-item #33 15 ก.ย. 69):

> **ทำไมต้องเลื่อน (open-item #33):** GitHub ไม่รันตรงเวลา config — วัด `gh run list --workflow update-prices.yml` ย้อนหลัง 30 รอบ (15 ก.ย. 69) พบ skew (เวลารันจริง − เวลา config) นิ่งที่ **4.30–4.61 ชม.** ในช่วง 12 วันล่าสุด (2–13 ก.ย. 69) เวลาเดิม 00:17 UTC จึงลงจริง ~04:40–04:53 UTC = **กลางช่วง SET เปิดตลาด** (SET เปิด 10:00 น. ไทย = 03:00 UTC) ทำให้ guard `isIntradayQuote` ข้ามหุ้นไทยทั้งชุดทุกวันธรรมดา — หุ้นไทย refresh ได้จริงเฉพาะรอบวันเสาร์เท่านั้น (ดู `docs/open-items.md` #33) · สัปดาห์ก่อนหน้า (30–31 ส.ค. 69) skew ขยับถึง 5.41 ชม. และมีเหตุการณ์ผิดปกติ 3 วันติด (27–29 ส.ค. 69) ที่ skew พุ่งถึง 6.9–11.2 ชม. — ไม่ใช้เป็นฐานตั้งเวลาใหม่ (ทำให้ไม่มีเวลา config ใดครอบคลุมได้ในหน้าต่าง 6 ชม. ที่มี) แต่เป็นความเสี่ยงคงเหลือที่ `isIntradayQuote` guard ยังกันไว้เป็นตาข่ายสุดท้ายถ้าเกิดซ้ำ · เวลาใหม่ 21:15 UTC เลือกจาก: ต้องอยู่**หลัง** 21:00 UTC (16:00 ET = US close ครอบทั้ง EDT/EST) และ**ก่อน** 03:00 UTC (SET เปิด) โดยเผื่อ margin ≥1 ชม. เหนือ skew ที่วัดได้ (4.61+1=5.61h) → actual run ตกช่วง 21:15–~02:51 UTC เสมอในสภาวะปกติ
