...
npm run verify:cron            # ประตู cron <!-- gen:verify-cron-steps -->5<!-- /gen:verify-cron-steps --> ขั้น (เฉพาะสิ่งที่ cron แตะ) — verify เต็มรันโดย ci-verify หลัง push
