...
- **แถว v3 ในคิว** (preflight พิมพ์บรรทัด `v3 <SYM>: …` ต่อใบตาม bucket — Plan 4a · pre-patch มือ = `node tools/update-prices.js --write --force <SYM>` **ตอนตลาดปิดแล้วเท่านั้น** (`--force` ข้าม guard intraday)): `mos-sign-flip` (PREPATCH) = **หลัง** `ship --prepatch`: pre-patch **มือ** → `npm test -- <SYM>` → controller commit เอง `price: pre-patch <SYM> (v3 flip)` + push ตาม CLAUDE.md §5 (**ไม่ผ่าน `ship`** — pre-patch ราคาไม่แตะ `meta.analysisDate` ⇒ `postcheck` ได้ review (footer ≠ วันนี้) แล้ว `postcheckGuard` ปฏิเสธ · `--force` = commit ติดป้าย `analyze:` + trailer ผู้วิเคราะห์เดิม ผิด · pre-patch ก่อน `ship --prepatch` = `.json` สกปรก → ทั้งชุดถูกปฏิเสธ · `ship --prepatch` ยังรับเฉพาะ `.html` — pre-patch อัตโนมัติของใบ v3 = Plan 4b · open-items 66) · LIGHT/FULL = **หลัง** `ship --prepatch`: pre-patch **มือ**ถ้าราคายังไม่สด (ตลาดปิดแล้วเท่านั้น) แล้ว `prep <SYM>` ตามปกติ (worker: `report.js export/save` — STEP 5U) · แถวอื่น (skip/DELIST/PLUMBING/REJECTED/UNKNOWN) ไม่มีบรรทัด
