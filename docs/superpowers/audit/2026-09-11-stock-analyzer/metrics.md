...
ตัวเลขอื่นที่ขยับในระยะ 0: ขั้น `npm run verify` **13 → 14** (+`queue-test`) · fixture ที่ผูกไฟล์รายงานจริง **2 → 0** (`test/fixtures/`) · รหัส E/W ใหม่ **0** (spec §8 ห้ามเพิ่ม E ก่อน quarantine ครบ) · `docs/open-items.md` เปิด **20** (รวม #22 #23 จาก final review) ปิดแล้ว **3**
