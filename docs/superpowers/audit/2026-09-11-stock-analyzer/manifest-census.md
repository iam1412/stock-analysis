...
ยิงทั้งคลัง 908 ใบ (warning รวม 149 → **358** · error คงเดิม 0 · **ผลของ E/W เดิมทุกรหัสไม่เปลี่ยนแม้แต่ไบต์เดียว** —
...
| **W22** คู่ consistency ไม่ตรง | **88** | f12 วันที่ disclaimer ↔ วันที่ราคา **65** · f47 legend ↔ FV 16 · f24 เป้าบน gauge ↔ การ์ดเป้า 4 · f49 mFair ↔ FV 2 · f54 vcell กรอบ ↔ .fv-box 2 · f46 fairLine ↔ FV 2 · f48 โซนสะสม ↔ FV **1** · กฎ roe **0** — รวม 92 ครั้ง |
...
2. **`notes` ของ cron — เตือนเฉพาะ "อ่านออกแต่เขียนไม่ได้"** (UNVERIFIED WRITE → 0):
   - disclaimer "ราคา ณ": cron เขียนได้ **412/908** · ที่เหลือ 496 แยกเป็น **431 ใบไม่มีวันที่ใน `.disc` เลย**
     (ไม่มีอะไรให้เขียน = ปกติ · f12 เป็น optional ตาม census 48.9%) กับ **65 ใบที่ `findPriceDate` อ่านออก
     แต่ regex ตัวเขียนจับไม่ได้** (รูปที่มีเลขเงินคั่น เช่น AIG "…(ราคา $79.39 · 2 ก.ค. 2569 …)" ซึ่ง `[^0-9<]` ข้ามไม่ได้)
     ⇒ **note ยิงเฉพาะ 65 ใบหลัง** — และ **63/65 ใบซ้อนทับกับ W22 คู่ f12↔f10 พอดี** (ต่างแค่ FCX ที่เขียนได้แต่ยังไม่ตรง
     และ SSP ที่บังเอิญตรงอยู่แล้ว) ⇒ ทุกบรรทัด note มีของจริงและมี W22 รับรอง (ถ้าเตือนทุกใบที่ไม่พบ = 496 บรรทัด
     ซึ่ง 87% แปลว่า "ใบนี้ไม่มีประโยคนั้น") — คำตัดสิน controller 12 ก.ย. 69
