...
อัปเดต: 15/9/2569 03:13:46 (Asia/Bangkok)

| ผล | ใบ |
|---|---|
| ย้ายได้ | 883 |
| ยังไม่ได้ (residue) | 25 |
| รวม | 908 |
...
| site mFair match ≠ N | 1 | FANG |
| cron-diff v1-unstable gate-warn:W22 | 1 | LWLG |
| cron-diff gate-warn:W24 | 1 | MU |
...
| site | ใบ | ตัวอย่าง (≤10) |
|---|---|---|
| psCard — ไม่มีการ์ดที่ตัดสินได้ | 860 | A AAI AAON AAPL ABBNY ABBV ABNB ABT ACE ACMR … |
| yieldCard — รูปตัวเลขต่าง ("N%" ≠ "N%") | 510 | A AAON AAPL ABBNY ABBV ABT ACN ADI ADM ADP … |
| peCard — ไม่มีการ์ดที่ตัดสินได้ | 424 | A AAOI ABNB ABT ACE ADP ADVANC ADVICE AEE AEHR … |
| pbvCard — ไม่มีการ์ดที่ตัดสินได้ | 368 | AAOI AAPL ABBNY ABBV ACE ACMR ACN ADBE ADI ADP … |
| yieldCard — ไม่มีการ์ดที่ตัดสินได้ | 253 | AAOI ABNB ACE ACMR ADBE ADSK AEHR AEVA AIG AKAM … |
| mcapCard — รูปตัวเลขต่าง ("$NB" ≠ "$NB") | 224 | AAON ABBNY ABBV ABNB ACMR ACN ADBE ADP AEE AIT … |
| scnNote — ไม่พบ | 221 | AAOI AAPL ABNB ACMR ACN ADBE ADI ADSK AEHR AEVA … |
...
| tgtCard — ไม่พบรูป "เป้า (+%)" | 101 | AAOI AAPL ABBNY ACMR ACN ADBE ADI ADSK AEHR ALAB … |
| zone — ราคาโซนสะสม N ≠ FV N (ผู้เขียนตั้งเอง) | 90 | AAOI ACMR ACN ADBE ADI ADSK ALAB ANET APH APP … |
| mcapCard — รูปตัวเลขต่าง ("฿N ล้าน" ≠ "฿N พันล้าน") | 65 | AAI AHC ANI APURE ASIAN ASK ASW BAFS BBIK BE8 … |
| mcapCard — รูปตัวเลขต่าง ("฿N ล้าน" ≠ "฿N หมื่นล้าน") | 47 | ACE AURA AWC AYUD BAM BCH BKI BLA CHG CK … |
...
| mcapCard — รูปตัวเลขต่าง ("฿NB" ≠ "฿N หมื่นล้าน") | 10 | AMATA BJC BTS DOHOME KTC M PLANB ROJNA SAUCE TVO |
| mcapCard — รูปตัวเลขต่าง ("$N" ≠ "$NB") | 10 | B DTE EIX ES EVRG FE LNT NI WEC XEL |
| mcapCard — รูปตัวเลขต่าง ("฿N" ≠ "฿N พันล้าน") | 9 | ADVICE AMATAV AU INSET RJH RPH S SICT SMPC |
...
| mcapCard — รูปตัวเลขต่าง ("$N ล้านล้าน" ≠ "$NT") | 8 | AAPL AMZN GOOGL META MSFT MU NVDA TSLA |
...
| fvBoxRange — กรอบใน fv-box ไม่ตรง values/ไม่มีสัญลักษณ์สกุลเงินครบ | 6 | AAOI BGC CAT LWLG OKJ TSEM |
...
| ชนิด | จุด (คอลัมน์) | ใบ |
|---|---|---|
| รูปเลขเปลี่ยนแต่ค่าเท่าเดิม (form) | 411 | — |
...
| ผ่าน | 865 |
| ตก (ไม่เขียน) | 2 |
| ยอมรับทั้งที่ตัวเทียบไม่ผ่าน (v2-as-baseline / ย้ายภายหลัง) | 4 |
| ผ่านแต่ต่างแค่รูป | 715 |

### ยอมรับทั้งที่ตัวเทียบไม่ผ่าน — `--v2-baseline` (ระยะ 3 Task 5) หรือย้ายภายหลังนอกสำมะโน

เกณฑ์ "v2 = v1" ใช้ไม่ได้เมื่อฝั่งที่ไม่นิ่งคือ v1 ⇒ แทนด้วย "v2 เองเสถียรบนกริดเดิม" (`v2Stability`) — เขียนแล้ว ไม่ใช่ residue
(คอลัมน์ "ทาง" = `v2-baseline` เมื่อรอบที่ย้ายบันทึกสำมะโนไว้เอง · `ย้ายภายหลังนอกสำมะโน` เมื่อรู้จากไฟล์จริงตอนรีรัน)

| ใบ | เหตุผลเดิมของตัวเทียบ | ทาง | ฝั่งที่ไม่นิ่ง |
|---|---|---|---|
| BNY | `cron-diff v1-unstable meta:dividendYield,visible` | ย้ายภายหลังนอกสำมะโน | ×1.09→×1.095: meta:dividendYield=v1 visible=v1 |
| PTG | `cron-diff v1-unstable gate-warn:W22` | ย้ายภายหลังนอกสำมะโน | ×0.855→×0.86: gate-warn:W22=v1 |
| THCOM | `cron-diff v1-unstable gate-warn:W22` | ย้ายภายหลังนอกสำมะโน | ×0.97→×0.975: gate-warn:W22=v1 |
| WWD | `cron-diff v1-unstable visible` | ย้ายภายหลังนอกสำมะโน | ×1.135→×1.14: visible=v1 |

### ตกตามชนิด

| ชนิด | ใบ | รายชื่อ |
|---|---|---|
| gate-warn:W22 | 1 | LWLG |
| gate-warn:W24 | 1 | MU |
...
|---|---|---|---|
| ค่าต่างที่มีอยู่ก่อน patch (migration ยอมรับ) | 35 | 0.99% | AEE ANI BF-B BWXT CBOE CHAYO CMS DD DUK EPG EQIX EXE FDS GLOBAL HSY IIG IVL JAZZ LDOS LPH MFC MMS PRINC RYAN SBAC SECURE SKR SNX SPCX SYM TGH THCOM TRMB TRU WICE |
...
- THCOM: «เหมาะสม ฿11.45 ฿ ▯ MOS 30% ฿9.16 MOS 20% ฿1» 8.02 → 8.01 (0.12%)
...
- LWLG — `cron-diff v1-unstable gate-warn:W22` · side v1 (×0.865→×0.87: gate-warn:W22=v1) · ×0.87 ราคา 4.55: warning v1 [W22] ≠ v2 [] — v1 W22 ป้าย gauge mCur "ปัจจุบัน $px" ↔ .px ราคา header: 4.6 ≠ 4.55
- MU — `cron-diff gate-warn:W24` · side unknown (ตกที่จุดแรกของ grid — ไม่มีจุดข้างเคียงให้เทียบ) · ×0.85 ราคา 828.97: warning v1 [W21] ≠ v2 [W21,W24] — v2 W24 อ่านวันที่วิเคราะห์จาก footer ไม่ได้ — "ข้อมูล ณ 24 มิ.ย.–13 ส.ค. 2026" ⇒ E44 ตัดสินไม่ได้ว่าเป็นใบใหม่ (ข้ามทั้งใบ) · ใบนี้มี prose ผูกราคา 5 จุด · แก้เป็นวันที่เดียวที่ parse ได้ ("14 ก.ย. 2569") — ช่วงข้ามเดือน/เดือนล้วน ตัวอ่านไม่รับ
...
|---|---|
| ย้ายเป็น v2 | **883/908** |
| residue (คง v1) | **25** = migrator 23 + cron-diff 2 |
| ในนั้น: ย้ายภายหลังโดยรอบที่ไม่ได้บันทึกสำมะโน | **18** — AEM AMKR BABA BNY BTG JMT MPC MTI MXL PTG RS SCGD SNNP STX THCOM TMAN VRT WWD |
| ในนั้น: ไม่มีบันทึก cron differential (รอบที่ย้ายไม่ได้เขียนสำมะโน) | **14** — AEM AMKR BABA BTG JMT MPC MTI MXL RS SCGD SNNP STX TMAN VRT |
| แบตช์ที่บันทึก | 0:95 · (ไม่ระบุ):43 · 1:97 · 2:97 · 3:96 · 4:97 · 5:96 · 6:92 · 7:93 · 8:94 · 9:8 |
...
<details><summary>เหตุผล residue เต็มรายใบ (25)</summary>
...
| LWLG | cron-diff v1-unstable gate-warn:W22 |
| MU | cron-diff gate-warn:W24 |
...
| หมวด | ใบ | หมายเหตุ |
|---|---|---|
| cron differential: ตรวจ / ผ่าน / ตก | 871 / 865 / 2 | ตก = LWLG MU (ไม่เขียน · รายละเอียดในหัวข้อด้านบน) |
| ยอมรับทั้งที่ตัวเทียบไม่ผ่าน (v2-as-baseline / ย้ายภายหลัง) | 4 | BNY PTG THCOM WWD |
| ค่าต่างที่มีอยู่ก่อน patch (migration ยอมรับ) | 35 | มากสุด 0.99% · AEE ANI BF-B BWXT CBOE CHAYO CMS DD DUK EPG EQIX EXE FDS GLOBAL HSY IIG IVL JAZZ LDOS LPH MFC MMS PRINC RYAN SBAC SECURE SKR SNX SPCX SYM TGH THCOM TRMB TRU WICE |
| รูปทศนิยมหมวด 6 เปลี่ยน (เช่น 3.5→3) | 695 | รอเจ้าของตัดสินว่าเป็นรูปหรือค่า (นโยบายปัด fmtMos เดียว) · รายชื่อด้านล่าง |
| %/ปี หมวด 6 ค่าขยับ (v2 คิด %/ปี จาก "รวม" ที่ปัดแล้ว) | 53 | 57 คอลัมน์ · รูปอย่างเดียว 411 คอลัมน์ · รายใบอยู่ในหัวข้อ %/ปี ด้านบน |
| รูปตัวเลขอื่น (ราคา/หน่วยเงิน/ศักราช) | 149 | |
| สีช่อง .ret ของ v1 ค้าง — v2 คิดสีตามเครื่องหมาย | 491 | |

<details><summary>รูปทศนิยมหมวด 6 เปลี่ยน — 695 ใบ</summary>
