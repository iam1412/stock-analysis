...
อัปเดต: 14/9/2569 18:22:59 (Asia/Bangkok)

| ผล | ใบ |
|---|---|
| ย้ายได้ | 670 |
| ยังไม่ได้ (residue) | 30 |
| รวม | 700 |
...
|---|---|---|
| legend แสดง N แต่ FV = N | 10 | COHU CTAS DTM DXCM FORM GNTX HMPRO LPLA RELX SHANG |
| จำนวนช่อง %/ปี ไม่เท่าเดิม | 4 | ACN JMT RS SCGD |
| ค่าไม่ตรงชั้น N | 3 | AMKR LWLG MTI |
...
| FV ไม่ตรงกันเอง | 1 | PNC |
| cron-diff v1-unstable gate-warn:W22 | 1 | PTG |
...
| site | ใบ | ตัวอย่าง (≤10) |
|---|---|---|
| psCard — ไม่มีการ์ดที่ตัดสินได้ | 670 | A AAI AAON AAPL ABBNY ABBV ABNB ABT ACE ACMR … |
| yieldCard — รูปตัวเลขต่าง ("N%" ≠ "N%") | 397 | A AAON AAPL ABBNY ABBV ABT ACN ADI ADM ADP … |
| peCard — ไม่มีการ์ดที่ตัดสินได้ | 339 | A AAOI ABNB ABT ACE ADP ADVANC ADVICE AEE AEHR … |
| pbvCard — ไม่มีการ์ดที่ตัดสินได้ | 295 | AAOI AAPL ABBNY ABBV ACE ACMR ACN ADBE ADI ADP … |
| yieldCard — ไม่มีการ์ดที่ตัดสินได้ | 201 | AAOI ABNB ACE ACMR ADBE ADSK AEHR AEVA AIG AKAM … |
| scnNote — ไม่พบ | 183 | AAOI AAPL ABNB ACMR ACN ADBE ADI ADSK AEHR AEVA … |
| mcapCard — รูปตัวเลขต่าง ("$NB" ≠ "$NB") | 179 | AAON ABBNY ABBV ABNB ACMR ACN ADBE ADP AEE AIT … |
| pbvCard — รูปตัวเลขต่าง ("N" ≠ "N") | 114 | A AAON ABNB ABT AEP AME AMGN AON AOS APD … |
| peCard — รูปตัวเลขต่าง ("N" ≠ "N") | 108 | AAPL ABBNY ACMR ACN ADI ADSK AMAT AMD ANET AOT … |
| scn | 106 | AAI ADM ADVICE AEE AEONTS AFL AIG AMATAV AME AON … |
| tgtCard — ไม่พบรูป "เป้า (+%)" | 82 | AAOI AAPL ABBNY ACMR ACN ADBE ADI ADSK AEHR ALAB … |
| zone — ราคาโซนสะสม N ≠ FV N (ผู้เขียนตั้งเอง) | 78 | AAOI ACMR ACN ADBE ADI ADSK ALAB AMKR ANET APH … |
| mcapCard — รูปตัวเลขต่าง ("฿N ล้าน" ≠ "฿N พันล้าน") | 46 | AAI AHC ANI APURE ASIAN ASK ASW BAFS BBIK BE8 … |
| mcapCard — รูปตัวเลขต่าง ("฿N ล้าน" ≠ "฿N หมื่นล้าน") | 32 | ACE AURA AWC AYUD BAM BCH BKI BLA CHG CK … |
| mcapCard — ไม่มีการ์ดที่ตัดสินได้ | 26 | ARM ASML AVGO BIDU BNS BRK-B CB DB DEO EPD … |
| mcapCard — รูปตัวเลขต่าง ("฿N พันล้าน" ≠ "฿N หมื่นล้าน") | 19 | AP BCP BCPG BEM BGRIM CBG CENTEL EGCO EPG GLOBAL … |
| mcapCard — รูปตัวเลขต่าง ("฿NB" ≠ "฿N หมื่นล้าน") | 10 | AMATA BJC BTS DOHOME JMT KTC M PLANB ROJNA SAUCE |
| mcapCard — รูปตัวเลขต่าง ("฿N" ≠ "฿N พันล้าน") | 8 | ADVICE AMATAV AU INSET RJH RPH S SCGD |
...
| mcapCard — รูปตัวเลขต่าง ("$N พันล้าน" ≠ "$NB") | 6 | BMO FTS IMO KGC RL RY |
| psCard — รูปตัวเลขต่าง ("N" ≠ "N") | 5 | AEHR CPW DRS EXPE IVL |
| mcapCard — รูปตัวเลขต่าง ("฿NB" ≠ "฿N พันล้าน") | 5 | BGC DITTO ILM PRINC SECURE |
| mcapCard — รูปตัวเลขต่าง ("$N B" ≠ "$NB") | 4 | ADM AIG BG C |
| mcapCard — รูปตัวเลขต่าง ("฿N ล้าน" ≠ "฿N แสนล้าน") | 4 | CRC KKP PTTEP SCC |
...
| mcapCard — รูปตัวเลขต่าง ("฿NM" ≠ "฿N พันล้าน") | 1 | NCAP |
...
| ชนิด | จุด (คอลัมน์) | ใบ |
|---|---|---|
| รูปเลขเปลี่ยนแต่ค่าเท่าเดิม (form) | 302 | — |
| ค่าจริงขยับ (value) | 65 | 44 |
...
|---|---|---|---|
| ค่าต่างที่มีอยู่ก่อน patch (migration ยอมรับ) | 26 | 0.99% | AEE ANI BF-B BWXT CBOE CHAYO CMS DD DUK EPG EQIX EXE FDS GLOBAL HSY IIG IVL JAZZ LDOS LPH MFC MMS PRINC RYAN SBAC SECURE |
