...
อัปเดต: 14/9/2569 18:24:15 (Asia/Bangkok)

| ผล | ใบ |
|---|---|
| ย้ายได้ | 763 |
| ยังไม่ได้ (residue) | 37 |
| รวม | 800 |
...
|---|---|---|
| legend แสดง N แต่ FV = N | 12 | COHU CTAS DTM DXCM FORM GNTX HMPRO LPLA RELX SHANG … |
...
| site vcellTgt match ≠ N | 2 | BABA STX |
...
| site | ใบ | ตัวอย่าง (≤10) |
|---|---|---|
| psCard — ไม่มีการ์ดที่ตัดสินได้ | 764 | A AAI AAON AAPL ABBNY ABBV ABNB ABT ACE ACMR … |
| yieldCard — รูปตัวเลขต่าง ("N%" ≠ "N%") | 451 | A AAON AAPL ABBNY ABBV ABT ACN ADI ADM ADP … |
| peCard — ไม่มีการ์ดที่ตัดสินได้ | 388 | A AAOI ABNB ABT ACE ADP ADVANC ADVICE AEE AEHR … |
| pbvCard — ไม่มีการ์ดที่ตัดสินได้ | 328 | AAOI AAPL ABBNY ABBV ACE ACMR ACN ADBE ADI ADP … |
| yieldCard — ไม่มีการ์ดที่ตัดสินได้ | 227 | AAOI ABNB ACE ACMR ADBE ADSK AEHR AEVA AIG AKAM … |
| scnNote — ไม่พบ | 202 | AAOI AAPL ABNB ACMR ACN ADBE ADI ADSK AEHR AEVA … |
| mcapCard — รูปตัวเลขต่าง ("$NB" ≠ "$NB") | 197 | AAON ABBNY ABBV ABNB ACMR ACN ADBE ADP AEE AIT … |
| pbvCard — รูปตัวเลขต่าง ("N" ≠ "N") | 126 | A AAON ABNB ABT AEP AME AMGN AON AOS APD … |
| scn | 125 | AAI ADM ADVICE AEE AEONTS AFL AIG AMATAV AME AON … |
| peCard — รูปตัวเลขต่าง ("N" ≠ "N") | 119 | AAPL ABBNY ACMR ACN ADI ADSK AMAT AMD ANET AOT … |
| tgtCard — ไม่พบรูป "เป้า (+%)" | 88 | AAOI AAPL ABBNY ACMR ACN ADBE ADI ADSK AEHR ALAB … |
| zone — ราคาโซนสะสม N ≠ FV N (ผู้เขียนตั้งเอง) | 86 | AAOI ACMR ACN ADBE ADI ADSK ALAB AMKR ANET APH … |
| mcapCard — รูปตัวเลขต่าง ("฿N ล้าน" ≠ "฿N พันล้าน") | 63 | AAI AHC ANI APURE ASIAN ASK ASW BAFS BBIK BE8 … |
| mcapCard — รูปตัวเลขต่าง ("฿N ล้าน" ≠ "฿N หมื่นล้าน") | 45 | ACE AURA AWC AYUD BAM BCH BKI BLA CHG CK … |
| mcapCard — ไม่มีการ์ดที่ตัดสินได้ | 27 | ARM ASML AVGO BIDU BNS BRK-B CB DB DEO EPD … |
| mcapCard — รูปตัวเลขต่าง ("฿N พันล้าน" ≠ "฿N หมื่นล้าน") | 23 | AP BCP BCPG BEM BGRIM CBG CENTEL EGCO EPG GLOBAL … |
| mcapCard — รูปตัวเลขต่าง ("฿N" ≠ "฿N พันล้าน") | 11 | ADVICE AMATAV AU INSET RJH RPH S SCGD SICT SMPC … |
| mcapCard — รูปตัวเลขต่าง ("฿NB" ≠ "฿N หมื่นล้าน") | 10 | AMATA BJC BTS DOHOME JMT KTC M PLANB ROJNA SAUCE |
| mcapCard — รูปตัวเลขต่าง ("$N" ≠ "$NB") | 8 | B DTE EIX ES EVRG FE LNT NI |
| mcapCard — รูปตัวเลขต่าง ("฿N พันล้าน" ≠ "฿N แสนล้าน") | 8 | BAY CPALL CPAXT GULF IVL MINT SCB SCGP |
| psCard — รูปตัวเลขต่าง ("N" ≠ "N") | 7 | AEHR CPW DRS EXPE IVL SNOW THREL |
| disc | 7 | AMATAV CKP DOHOME FNF PRINC RAM RLI |
| mcapCard — รูปตัวเลขต่าง ("฿NB" ≠ "฿N พันล้าน") | 7 | BGC DITTO ILM PRINC SECURE SNC TK |
| mcapCard — รูปตัวเลขต่าง ("$N พันล้าน" ≠ "$NB") | 7 | BMO FTS IMO KGC RL RY TD |
| mcapCard — รูปตัวเลขต่าง ("฿N ล้าน" ≠ "฿N แสนล้าน") | 7 | CRC KKP PTTEP SCC THAI TLI TOP |
| fvBoxRange — กรอบใน fv-box ไม่ตรง values/ไม่มีสัญลักษณ์สกุลเงินครบ | 6 | AAOI BGC CAT LWLG MXL OKJ |
| mcapCard — รูปตัวเลขต่าง ("$N ล้านล้าน" ≠ "$NT") | 6 | AAPL AMZN GOOGL META MSFT NVDA |
...
| mcapCard — รูปตัวเลขต่าง ("฿NM" ≠ "฿N พันล้าน") | 2 | NCAP SUN |
...
| ชนิด | จุด (คอลัมน์) | ใบ |
|---|---|---|
| รูปเลขเปลี่ยนแต่ค่าเท่าเดิม (form) | 337 | — |
| ค่าจริงขยับ (value) | 83 | 54 |
...
|---|---|---|---|
| ค่าต่างที่มีอยู่ก่อน patch (migration ยอมรับ) | 31 | 0.99% | AEE ANI BF-B BWXT CBOE CHAYO CMS DD DUK EPG EQIX EXE FDS GLOBAL HSY IIG IVL JAZZ LDOS LPH MFC MMS PRINC RYAN SBAC SECURE SKR SNX SPCX SYM TGH |
