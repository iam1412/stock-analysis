...
- **ไม่มี session ใหม่ = ไม่เขียน ไม่ flag ไม่ error (R5)**: วันที่ของ quote ≤ `market.priceDate` และ `round2(price)` = `market.px` → "ไม่เปลี่ยน" (นับในยอด "ไม่เปลี่ยน") · เคสจริง = ICC (หุ้นไทยสภาพคล่องต่ำ — `regularMarketTime` ค้างที่วันซื้อขายล่าสุด) · `priceDate` ไม่ถอยหลัง
...
- **flags/บันทึก (R7)**: `evaluated` · `aliveAsserted` รวมใบ v3 (ไม่งั้น flag เก่าของใบ v3 ไม่มีวันเคลียร์) · `reportExists` = `RS.symbols` (Plan 2b) · commit body + บรรทัดสรุปนับทั้งสองสาย + บรรทัด `v3-lane: N ใบ · …` (token grep ได้ใน log ของ Actions) · `update-prices.yml` นับ `reports/*.json` ในชื่อ commit + คอมเมนต์หัวไฟล์ · `v3Guard` / `v3Refusal` / `v3SweepNotice` ถูกถอด พร้อมเทสที่ตรึงพฤติกรรม P4 (`scanners-cron.test.js` · `update-prices-test.js:30` · `queue-test.js:1570`) และ stock-controller `SKILL.md` §9 ใน PR เดียวกัน · อ่านใบ v3 ไม่ได้ = `failed` เหตุผล `no-stock-meta` (bucket PLUMBING เดิม — ไม่เพิ่ม reason ใหม่)
- **คำสั่งมือ (R7 · R8)**: `--write --force <v3 SYM>` = **เขียนจริง** (pre-patch มือของแถว `mos-sign-flip` ใบ v3) · `--heal-derived <v3>` = exit 1 พร้อมเหตุผล (ใบ v3 ไม่มีค่า derive ค้างให้ซ่อม — ห้าม exit 0 เงียบ) · sweep `--heal-derived` ข้ามใบ v3 พร้อมบรรทัดนับ · **pre-patch อัตโนมัติของแถว v3** (`preflight` `patchTargets.skippedV3` · `parseGateFailures` ที่จับแต่ `.html` · ทาง revert · `ship --prepatch` ที่ปฏิเสธ `.json`) **= P6** — P5 ให้ preflight/triage ไม่ล้มบนแถว v3 และพิมพ์หนึ่งบรรทัดต่อใบ: "v3 <SYM>: controller → `node tools/update-prices.js --write --force <SYM>` (flip) หรือ export/save (P6 queue flow)"
- **gate ของ cron (R9 · ปิด #61)**: `verify:cron` = `check-reports → check-v3 → build → build-test → engine-exec → check-site` (วัด ~0.1 วิ) · `node tools/gen-docs.js` สร้างเลขขั้น/ลำดับใน CLAUDE.md §8 + `docs/price-refresh.md` ใหม่ (`docs-test --check` บังคับ)
- **ความเสี่ยงที่ยอมรับ (R2)**: `_sig` อยู่บรรทัดท้ายไฟล์ ⇒ `save` ของคนบน main วันเดียวกับ cron = conflict ตอน `pull --rebase` (คลาสเดียวกับแก้ใบ v2 วันเดียวกัน) · yml retry 3 รอบเหมือนเดิม · กติกา: **ใบ v3 ที่ save วันเดียวกับ cron → ship ก่อน 04:00** (`docs/price-refresh.md`) · ทบทวนที่ P6 เมื่อ save เป็นงานรายวัน
