...
- **UPDATE**: `report.js export` → แก้ draft → `save` · `update-prices --write --force <SYM>` บนใบ v3 = **เขียน `market.*` จริง** ตั้งแต่ Plan 3 (§7 R8 — ใน P4 เคย exit ≠0 ตาม §6.5/#62) · flow คิวของ v3 UPDATE (`prep`/`postcheck`/`ship`) = P6 — `prep` ปฏิเสธ symbol v3 ("v3 UPDATE = P6 …")
...
- `prep` **ปฏิเสธ symbol ที่เป็น v3 แล้ว** ("v3 UPDATE = P6 …" — ก่อน Plan 3 ข้อความคือ "v3 UPDATE = Plan 3") — ไม่ทำเหมือนเป็น NEW
...
11. **อินพุตของ init = sidecar `.queue/prep/<SYM>.json` เท่านั้น** (ไม่ regex `.md`) · `market` ไม่เข้า `.work/` · `init` ปฏิเสธเมื่อมี `.html` หรือ `.json` · `prep` ปฏิเสธ symbol v3 ("v3 UPDATE = Plan 3" → "v3 UPDATE = P6 …" ใน Plan 3) · `export` ส่งเป็นคำสั่ง แต่ flow คิวของ v3 UPDATE = P6 — *advisor 24 ก.ย. 69 (เจ้าของมอบอำนาจ)*
