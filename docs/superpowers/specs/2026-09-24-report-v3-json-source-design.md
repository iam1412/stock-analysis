...
| P5 cron | สาย v3 ใน update-prices + `range52w` · **เส้นตาย (วัดจาก gate จริง 24 ก.ย. 69 — §6.6): แข็ง = `market.priceDate` ของใบ exit + 120 วัน (E27 ล้ม `npm run verify` ทั้ง repo ที่ +121) · เป้า = +45 วัน (W09 เตือนที่ +46)** (open-item #62 — ราคาใบ v3 แช่แข็งจนกว่า P5) · ห้ามรวมเข้า 2b/2c | dry-run บนใบ v3 ทั้งหมด = ไม่มี diff นอก market · รอบจริง 3 วันไม่มี patch-rejected ผิดปกติ |
