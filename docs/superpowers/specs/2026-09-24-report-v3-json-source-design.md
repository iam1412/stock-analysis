...
```jsonc
{
  ...
  "meta": {                                                   // worker
    ...
    "exchange": "NYSE", "headerTags": ["Animal Health"],     // ป้ายหัวรายงาน (ใบ NEW 0–2 · สคีมารับ ≤3 — ช่องที่ 3 สำหรับ ADR/dual listing ของใบ migrate · Plan 4b) — ไม่ใช่ tags.json
    "priceNote": "StockAnalysis.com ตรงกับ Yahoo Finance",     // วงเล็บหลัง "ราคา ณ" (ไม่บังคับ)
    "sectorLine": "NYSE · Healthcare · Medical Devices",      // ไม่บังคับ · prose ≤100 · ข้อความ gdots ที่ v2 แสดง → บรรทัดเล็กใต้ tags (Plan 4c-prep · §3.7 ข) · ไม่มี = byte-identical
  ...
  "legs": [                                                   // วิธีประเมิน 2–4 ขา — ค่าของขาคิดเอง
    { "method": "pe", "label": "P/E มัธยฐาน 5 ปี",
      ...
      // Plan 4c-prep (§3.7 ก): "inputs": { …, "base": "eps" | "epsForward" | "epsFy" } — เฉพาะ pe · default eps · **input ของการคำนวณ** (compute อ่าน fundamentals ตามนี้ · ขา forward live ใต้ cron) · ตัวเลขผู้เขียนต่าง = "override": { "epsForward": … }
      // "baseLabel": "FY2026E consensus" — ระดับขา นอก inputs · plain ≤24 · ถ้อยคำงวดต่อท้ายป้ายฐาน · "label" ≤80 เฉพาะใบ migrate (meta.migratedFrom)
  ...
  "scenarios": {                                              // section 6 — Bear/Base/Bull ครบ 3
    ...
    "driver": "eps",                                          // eps | ffo | revenuePerShare | bvps | fcfPerShare | de | fre (4b) | ebitdaPerShare (4c-prep)
    "exitMetric": "pe",                                       // pe | ps | pbv | pffo | pfcf | evsales (4b) | evebitda (4c-prep · คู่กับ driver ebitdaPerShare เท่านั้น)
  ...
  "verdict": { "extraCells": [{ "k": "ปันผลต่อหุ้น (TTM)", "v": "{{dps}} (สม่ำเสมอ)" }] },  // ไม่บังคับ · ≤2 · vcell ที่ 3+ ของหมวด 8 (k plain · v prose) · Plan 4c-prep §3.7 ข
  ...
  // "text": { …, "legendNote": "จุดสูงสุด ก.ย. 26" }  — ไม่บังคับ · ≤80 · คำผู้เขียนเท่านั้น (ไม่มีเงิน/% /token/"ราคา <…>") → span ที่ 4 ของ legend หมวด 2 · Plan 4c-prep §3.7 ข
  "catalysts": ["…", "…"],                                    // 3–8 ข้อ
  "risks": ["…", "…"],                                        // 3–8 ข้อ
...
| `dcf` | tg, r, rfCurrency + **ทางใดทางหนึ่ง**: g1, years1 (2-stage) **หรือ** `stages: [{years, g}]` 1–10 ช่วง · Σ years ≤ 40 (N-stage · Plan 4b Task 2 · §13-3) (fcf/netDebt/shares มาจาก fundamentals หรือ override — สำเนาเดียว) | 2-stage มาตรฐาน · N-stage = FCF โตตาม g ของแต่ละช่วงต่อกัน แล้ว terminal ด้วย tg |
...
- `multipleSource` enum: `median5y` · `median10y` · `peer` · `justified` · `sector` · `author` (Plan 4b Task 6b — ตัวคูณที่ผู้เขียนกำหนดเอง ไม่ใช่มัธยฐาน/peer/sector: migrator ใช้เมื่อ mdesc ไม่ได้บอกที่มา ⇒ ไม่ติดป้าย "มัธยฐาน" ให้ผิด) · `current` ได้เฉพาะขา `role:'context'` (§13-6) — บนขา fv **ไม่มี `current`** ⇒ สมอตายแบบประกาศตรง = error ตอน save · สมอตายแฝง (multiple ห่าง px/eps หรือ forward ≤7%) = W18/W25 เป็น **การคำนวณ** ไม่ใช่ regex
...
- **ทำแล้ว (Plan 4b Task 1/2/6b)**: `scenarios.exitDp` (int 0–2 · ไม่มี = พิมพ์เหมือนเดิม) · `exitMetric` `evsales` · `driver` `de`/`fre` · ช่องข้อความผู้เขียนที่ไม่บังคับ `scenarios.hintNote` (ต่อท้าย hint หมวด 6 ก่อน `{{rd:scnNote}}` เดิม) และ `scenarios.cases[i].retNote` (ป้ายต่อท้ายผลตอบแทนของฉาก — ห้ามเป็นตัวเลข/ป้าย "ต่อปี" ที่จะอ่านผิดเป็นผลตอบแทนรวมของ v3 → ใบแบบนั้น = HUMAN) · ทั้งสองเป็น prose สั้น (token ได้ · ห้ามแท็ก)
- **ทำแล้ว (Plan 4c-prep Task 2 · §3.7 ก)**: `exitMetric` = `pe` · `ps` · `pbv` · `pffo` · `pfcf` · `evsales` · **`evebitda`** (ป้าย "EV/EBITDA" · `exitTarget` สูตรเดียวกับ `evsales`: `end × m − netDebt/shares` · เป้า ≤0 = named throw เดิม) · `driver` เพิ่ม **`ebitdaPerShare`** (ป้าย "EBITDA/หุ้น" · ฐาน = `fundamentals.ebitda / shares`) · **กฎคู่** `evebitda` ⇔ `ebitdaPerShare` (schema + compute ปฏิเสธคู่ผิด) · ใบ migrate ที่คู่ไม่ครบ = HUMAN (sweep `35b4be74e`: 1 ใบ)
...
| H | **ไม่มี markup token** · ใช้ `tone: 'pos'\|'neg'\|'neu'\|'none'` ต่อการ์ดแทน (`'none'` = ไม่มีคลาสสี — Plan 4b Task 1 · §10.3) (`metrics.cards[i]` เขียนเป็น `{ "key": "pbv", "tone": "pos" }` ได้ · `metrics.custom[i].tone`) → render เป็น class สีเดิม (`.v pos` ฯลฯ) | ต้องการสีค่าบวก/ลบบนการ์ดแบบใบเดิม แต่ไม่ขยาย whitelist/pill (กัน markup creep ที่ template เดียวตั้งใจปิด) — ป้าย pill ในข้อความตกเป็นข้อความธรรมดา | BBL G4 | pill ใน `.d` 124 · ใน `.mname` 93 |
...
- `legs[i].inputs.base: 'eps'|'epsForward'|'epsFy'` (ตัวเลือก · default `eps` · เฉพาะ `pe`) — **ตัวตั้งเป็น input ของการคำนวณ ไม่ใช่ป้าย** (advisor 26 ก.ย. 69): compute อ่านช่อง fundamentals ตามนี้ (ขา forward ยังคง live ใต้ cron/UPDATE แทนการแช่ตัวเลข forward ไว้ใน `override.eps`) · `override.epsForward` เมื่อตัวเลขผู้เขียนต่าง · render พิมพ์ป้ายฐานจาก `base` ("EPS FY2026E (forward)") + `legs[i].baseLabel` (≤24 · ระดับขา นอก `inputs` · เฉพาะถ้อยคำงวด เช่น "FY2026E consensus") · `label`/`note`/`baseLabel` อยู่ระดับขา นอก `inputs` ⇒ `L.legValue` + invariant 1771/1771 ไม่แตะ · ปิดคลาส "ขา P/E ฐาน forward พิมพ์ (TTM)" 17 ขา (+95 ขาที่พิมพ์ "EPS ปรับ" โดยไม่บอกว่า forward)

**ข. ช่องรับข้อความผู้เขียนที่ยังไม่มีที่ (Kind 3 — พาไป ไม่ทิ้ง)**
- `meta.sectorLine` (string ≤100 · prose) — ข้อความที่ผู้เขียนพิมพ์ในจุด gdots ของ header (NYSE · Healthcare · Medical Devices …) — **ตรวจแล้ว 26 ก.ย. 69: v2 แสดงข้อความนี้จริง** (`.gdots` เป็น inline-flex ไม่มีการซ่อนข้อความ · ข้อความตรงใน div แสดงเป็นตัวหนังสือปกติ) ⇒ ช่องนี้ = คืนสิ่งที่ v2 แสดง ไม่ใช่เพิ่ม · render เป็นบรรทัดเล็กใต้ tags เมื่อมี (ไม่มี = byte-identical) · 65 ใบ (คำ NYSE/SET ที่ซ้ำกับ `meta.exchange` ให้ containment ตัดสิน)
- `verdict.extraCells: [{k, v}]` (≤2 · prose) — vcell ที่ 3+ ของหมวด 8 · 11 ใบ
- `text.legendNote` (string ≤80) — คำเพิ่มใน legend หมวด 2 · 20 ใบ
- `scenarios.cases[i].retNote` รับคำหน้าตัวเลข % ด้วย ("Total"/"รวม") · `scenarios.perYear` อนุมานจากป้าย `/ปี|ต่อปี` ใน `.ret` เป็นค่า enum ที่มีอยู่แล้ว (`'cagr'` ค่าตั้งต้น · `'linear'` เมื่อทุกคอลัมน์ตรง total/N และไม่ตรง CAGR — คืนสิ่งที่ผู้เขียนพิมพ์ ไม่ใช่สูตรใหม่ · คู่ที่ไม่เข้าสูตรไหน → null = ไม่พิมพ์ %/ปี คำยังหลุด → HUMAN · advisor 26 ก.ย. 69 — เดิมเขียนว่า "อนุมานเป็น `true`" แต่ช่องเป็น enum ไม่ใช่ boolean) (CAGR = แบบแผนผู้เขียน 138/147 ใบที่สอดคล้องตัวเอง) · 77 ใบ
...
**ค. transform ที่อนุมัติเพิ่ม (Kind 2 — §10.2 ข้อ ง · ต้องมี guard)**: `SYNONYM_VOCAB` ปิด ผูกโซน + ผูกองค์ประกอบที่ align กัน (ไม่ใช่ทั้งหน้า) — คู่คำที่หมายถึงปริมาณเดียวกับที่ v3 พิมพ์ · **guard เชิงโครงสร้างทุกคู่**: ใช้ได้เฉพาะเมื่อองค์ประกอบ v3 ที่ align กันเป็นปริมาณเดียวกัน (แถวออกสำหรับ Exit · driver ต่อหุ้นสำหรับ sh · `ไม่จ่าย ≡ divCum 0` เฉพาะฉากที่ v3 ไม่มีปันผล · `ทรงตัว|คงที่|flat ≡ growth 0` เฉพาะ growth = 0) · คำ template ของ s6 ที่ v3 ไม่พิมพ์เลย (`รวม ×74` · `รวมปันผล` · `ปันผลสะสม` — ถ้า v3 ไม่มีป้าย "รวม 3 ปี" ต่อคอลัมน์) = `TEMPLATE_VOCAB` โซน s6 โดยนับจำนวนครั้งให้ตรง 3 คอลัมน์ ไม่ใช่ synonym · `.ret` แบบตัวเลข (11 ใบ): พาได้เมื่อ `classifyNumber` บอกว่าเท่ากับผลรวม v3 ภายในการปัดที่พิมพ์ ไม่งั้น residual · **การ align ขาใน gate (Task 6 M-4 ใช้ `A.labelOf`) ต้องใช้ป้ายด้านเดียวกันทั้งสองฝั่ง** เมื่อ assemble เขียน `legs[i].label` ของผู้เขียน (ไม่งั้น align เพี้ยนเงียบ) · s6 แถวออก `Exit|ทางออก ≡ ออก` · ตัวตั้งต่อหุ้น `sh|Sh|share|/sh|หุ้น ≡ /หุ้น` · driver `Rev|Revenue|Sales|รายได้ปี ≡ รายได้` (เฉพาะเมื่อ driver = revenuePerShare) · `FFO ≡ ป้าย ffoBasis` (เมื่อ ffoBasis ตรง) · ป้ายรวม `total|Total|รวม ≡ รวม N ปี` · s3 `มัธยฐานย้อนหลัง ≡ มัธยฐาน` · s3 ขา context `บริบท|ไม่รวมในกรอบ|ไม่รวมใน FV|ไม่นับใน ≡ (บริบท — ไม่นับใน FV)` เฉพาะขาที่ v3 role = context · **ห้ามใส่คำที่บอกข้อเท็จจริง** (AFFO/Core/forward/FY#E/Tangible/adj/GAAP) — reviewer ตรวจรายการทีละคำเหมือน `TEMPLATE_VOCAB`/`FORMULA_VOCAB`

**screenshot = acceptance ไม่ใช่ของประกอบ**: องค์ประกอบใหม่ที่มองเห็น 3 อย่าง (`sectorLine` · `extraCells` · `legendNote`) + กริด 8 การ์ด — PR body ต้องมีภาพ desktop + phone width อย่างละ 1 ใบต่อองค์ประกอบ · ช่องทุกช่องเป็นตัวเลือก ⇒ ทางย้อนกลับ = ถอดค่า
...
**จ. ไม่ทำในตัว migrator (residual → ถอดความ (draft → worker แก้ → adopt → ship --migrate · §10 ข้อ 4 · เจ้าของ 26 ก.ย. 69 "ห้ามวิเคราะห์ใหม่ … ย้าย V2 => V3"))**: ช่อง growth ที่เป็น prose ("ฟื้นตัวช้า" · "WTI $50/bbl") และ driver cell แบบข้อความ — ห้าม parse prose เป็นตัวเลข · E52 (sotp/nav ไม่มีตาราง) · ขานักวิเคราะห์เป็น fv · fv < 2 · era mismatch · sources < 3 · ค่าฉากไม่ครบ 3 คอลัมน์

**ฉ. อื่นในแผนเดียวกัน**: #67 — ~~`meta.aiModel` เข้า freshHash~~ **แก้ที่ชั้น `ship --prepatch` แทน** (advisor 26 ก.ย. 69: `freshHash` ไม่นับ `aiModel` เป็นการออกแบบตาม §8 ที่ `io.js` ประกาศไว้ — เปลี่ยนจะ restamp `updated` ของ OGE/ICC และทุกใบ migrate · จุดบกพร่องจริงของ #67 คือ `ship --prepatch` จัดการแก้ aiModel อย่างเดียวเป็น market-only ⇒ เพิ่ม `IO.prepatchHash` = canonical ไม่รวม `market`/`_sig` **แต่รวม `meta.aiModel`** ให้ `ship --prepatch` เทียบ · `freshHash`/`updated` ไม่ขยับ) · #68 (BDMS ขา 1 → author · TRMB medianWindow) · ~~ปัด `pe`/`dividendYield`/เปลี่ยน `name`/`title` ใน reports.json~~ **ไม่ทำ** (advisor 26 ก.ย. 69: index จัดรูปแบบตอน render ค่า raw ไม่มีผล · ชื่อแบบ v3 "(SYM)" เป็นแบบแผนที่ตั้งใจ index จะลู่เข้าหาเมื่อ migrate ครบ — PR แบตช์แรกใส่ screenshot การ์ด index ก่อน/หลัง) · **batch runner** `tools/migrate-v3.js batch <table.csv> --class <driftClass…> --n <push every N>`: อ่านตารางที่ advisor อนุมัติ → `convert --write --accept-drift` ใหม่ตอนแบตช์ → **ปฏิเสธใบที่ bucket/driftClass ต่างจากแถวที่อนุมัติ** → `ship --migrate` (CLEAN 50 ใบ/commit · อื่น 1 ใบ/commit) → push ทุก N commit

**เป้าที่วัดได้ (sweep หลัง 4c-prep)**: CLEAN ≥ 11 (ไม่ลด) · HUMAN 472 → **≈ 220–250** (ประมาณการ advisor 26 ก.ย. 69: a–g+x ≈118 + งานถ้อยคำขา/ฉาก ≈100–130) · DIST identical · ตัวเลขจริงลง PR body · residual = ถอดความ (draft → worker แก้ → adopt → ship --migrate · §10 ข้อ 4 · เจ้าของ 26 ก.ย. 69 "ห้ามวิเคราะห์ใหม่ … ย้าย V2 => V3") (ไม่มีเวฟวิเคราะห์ใหม่)

**ผลจริง (Task 6 · sweep `35b4be74e` · clean clone · analysis-px):** CLEAN **12** · VALUE-DRIFT **450** · HUMAN **447** (35 ล้มก่อนเทียบ) · TEXT LOST ใน CLEAN 0 · DIST identical — **พลาดเป้า HUMAN ≈220–250** (ลดจาก 472 แค่ 25) · เหตุ: ทุกกฎพา/synonym ถูกทำเป็น fail-closed เพื่อไม่ซ่อนป้ายผิดแม้ใบเดียว (คำตัดสิน Task 5 I-1/I-2/I-3 · option (ii) · comparator ฐาน รอบ 4/5 — ZS · CENTEL · HON · ZBH คง HUMAN โดยการออกแบบ) ⇒ กำไรจากช่อง/enum เชิงโครงสร้าง (HUMAN→VALUE-DRIFT 124 · HUMAN→CLEAN 2 = BIZ LPH) ถูกหักด้วยคำผู้เขียนที่ gate เห็นใหม่หลังถอด mask ของ 4b (VALUE-DRIFT→HUMAN 100 · CLEAN→HUMAN 1 = ONEE: note ขา pe มีคำ forward "FY2569e" ที่ `baseOf` ไม่ได้กิน ⇒ ไม่พาทั้ง note) — ตัวถือ HUMAN อันดับแรกคือ TEXT LOST s3 307 ใบ (208 ใบเหตุนี้อย่างเดียว · 2798/3316 คำอยู่ใน `.mdesc` = เหตุผลผู้เขียนเรื่องที่มาตัวคูณ/ฐาน forward ที่พาไม่ได้โดยไม่เสี่ยงป้ายฐานผิด) · residual ทั้งหมดไปถอดความ (draft → worker แก้ → adopt → ship --migrate · §10 ข้อ 4 · เจ้าของ 26 ก.ย. 69 "ห้ามวิเคราะห์ใหม่ … ย้าย V2 => V3") (§11 แถว 4c-residual)
- D-pin ที่ขยับระหว่างทำ (ไม่มีข้อไหนกลับทิศ — ทุกข้อแคบลงแบบ fail-closed · ที่มา `docs/decisions.md` §10 "Report v3 Plan 4c-prep"): **D3** `verdict.extraCells` ไม่รับ vcell ที่ key ซ้ำช่อง template/ช่องตระกูลนักวิเคราะห์ · ค่าที่เป็น token template ล้วน · ค่าผูกราคา (literal ราคา/ช่วง 52 สัปดาห์/yield/ตัวคูณปัจจุบัน) = H · `text.legendNote` รับคำล้วน (เงิน/%/token/"ราคา <…>" = H) · `qualifierOf` ทุกทางพา (รอบ 1/2/3 · segment หลัง · วงเล็บ · lead-in) บนขา pe/pfcf/pffo/ps/pbv ห้ามพาคำ forward/งวด/basis ที่ `baseOf` ไม่ได้กิน ยกเว้นคำ basis ที่ตรงฐานที่ v3 render · **D4** `TEMPLATE_COUNTED` นับเฉพาะตำแหน่งป้าย (`<li>` label · ท้าย `.ret`) ≤1 ต่อคอลัมน์ (ไม่ใช่ ≤3 ต่อโซน)
...
- **กลไกที่เลือก (Plan 4b D1 · Task 3)**: `meta.migratedFrom {updated, v2Hash}` ที่ migrator ประทับ (updated = แถว `HEAD:reports.json` · v2Hash = freshHash ของใบ v2) + `build.updatedFor(old, h, mf, now)`: hash ตรงแถวเดิม = คงค่า · **แถวเดิมยังเป็น `v2Hash` = คืน `mf.updated`** · อื่น = now ⇒ build แรกหลัง convert คง `updated` ของ v2 และบันทึก hash v3 · build ถัดไปคงเพราะ hash ตรง · แก้เนื้อหลังจากนั้น = ประทับ now ตามปกติ · `report.js save --light` ปฏิเสธการแตะ `meta.migratedFrom` (allowlist) · **ข้อควรรู้ (Task 3 review M2 · ตามการออกแบบ)**: แก้ใบก่อน build ครั้งแรกหลัง convert = แถวยังเป็น `v2Hash` ⇒ การแก้นั้น**ไม่ถูกประทับ** now (คง updated ของ v2) — runbook: **convert → build ทันที → ค่อยแก้** · **gate ผ่านแล้ว (Task 9 ซ้อมบนใบจริงใน scratch 25–26 ก.ย. 69 · CCET/LEN)**: build 1 คง updated v2 ทั้งคู่ · preserve-dates + build 2 = diff ของ `"updated"` 0 บรรทัด · `ship --migrate --no-push` · build 3 หลัง commit = reports.json สะอาด · แก้ CCET แล้ว build = ประทับ now (LEN คงเดิม) → `docs/decisions.md` §10 "Report v3 Plan 4b"
...
4. **ผลลัพธ์ 3 ถัง**: `CLEAN` (เขียน .json ลบ .html ใน commit เดียวกัน) · `VALUE-DRIFT` (ต่างเฉพาะตัวเลขที่ v2 ค้าง — controller รีวิวตาราง แล้วอนุมัติเป็นชุด) · `HUMAN` (ขาจำแนกไม่ได้ · weights ไม่ลง · ข้อความหาย · ขาเป้านักวิเคราะห์เป็น fv · fv <2 ขา · sources <3) — **Task 0 prototype (นิยามของ agent · `.superpowers/sdd/v3-plan4-task0/classify.csv`): CLEAN 290 · VALUE-DRIFT 519 · HUMAN 100** (custom >4 การ์ด 38 · scenario นอก enum 34 · ขานักวิเคราะห์ 17 · fv<2 16 · weights 9 · sources<3 4) — ตัวเลขนี้จะเลื่อนตามกฎข้างบน (weights/exit ไม่ back-solve ⇒ CLEAN→DRIFT บางส่วน · catalog keys/enum/rating nullable ⇒ HUMAN ~72 ใบ→CLEAN · HUMAN แท้ ≈33 = ขานักวิเคราะห์ 17 + fv<2 16 ซ้อนกัน + sources<3 4 — ประมาณการของ 4b · วัดหลัง 4c-prep (sweep `35b4be74e`): ขานักวิเคราะห์ ∪ fv<2 = **31** ใบ) · **การ sweep ของ 4b เป็นตัวตัดสิน**
   - **ผล sweep จริง (Plan 4b final-review fix round · migrator ที่ commit `1772355cd` (fix round 2) · clean clone · analysis-px 909/909 — 140 ใบได้ราคา ณ commit footer ชุด `6d4fd7ada` ไม่ใช่วันวิเคราะห์จริง)** → `docs/superpowers/specs/2026-09-25-v3-migration-sweep.md` (+ `.csv`): **CLEAN 11 · VALUE-DRIFT 426 · HUMAN 472** (ในนั้น 33 ใบล้มที่ compute/render ก่อนเทียบ) · TEXT LOST ใน CLEAN = 0 · VALUE-DRIFT ตาม `driftClass`: fv-rounding 20 · scn-tgt 35 · index 1 · prose-stale 11 · multi-rounding 108 · mixed 251 (129 ใบของ mixed มี maxDeltaPct ≤ 1) · **ก่อน fix round (`559fdca45`): 11 / 501 / 397** — gate เดิมทิ้งคำทุกคำใน mdesc ขา computed และยกเว้น TEMPLATE_VOCAB ทุก zone (final review I-1/I-2) ⇒ 58 ใบ VALUE-DRIFT → HUMAN (คำผู้เขียนในสูตรขา computed ที่หน้า v3 ไม่พก · SIRI/SMCI "(มัธยฐาน 5 ปี)" ในช่อง §6) · fix round 2 (re-review I-3: ฐาน forward/ประมาณการ/Tangible ออกจาก FORMULA_VOCAB) อีก 17 ใบ ⇒ รวม 75 · CLEAN 11 ใบเดิมทั้งหมด (LEN/WHA/CMCSA/XYL: note ของขา computed พกข้อความผู้เขียนครบแล้ว) · `--no-stale` 16 / 496 / 397 → 16 / 421 / 472 · ตัวเลข prototype ข้างบน (CLEAN 290) ใช้นิยามถังของ Task 0 เองที่ไม่มี rd/sm structured diff + word-level zone gate — ไม่ใช่ค่าที่เทียบกันได้ · ช่องว่างที่เหลือเป็นคำถามเจ้าของก่อน 4c (`docs/decisions.md` §10 "Report v3 Plan 4b")
   - **ผล sweep หลัง 4c-prep (Task 6 · migrator ที่ `35b4be74e` · clean clone · analysis-px — แทนที่ไฟล์ sweep เดิม)**: **CLEAN 12 · VALUE-DRIFT 450 · HUMAN 447** (35 ล้มก่อนเทียบ) · TEXT LOST ใน CLEAN 0 · VALUE-DRIFT ตาม `driftClass`: fv-rounding 21 · scn-tgt 26 · index 1 · prose-stale 12 · multi-rounding 108 · mixed 282 (139 ใบของ mixed มี maxDeltaPct ≤ 1) · เทียบ 4b (11 / 426 / 472): CLEAN→CLEAN 10 · HUMAN→CLEAN 2 (BIZ LPH) · CLEAN→HUMAN 1 (ONEE) · HUMAN→VALUE-DRIFT 124 · VALUE-DRIFT→HUMAN 100 · ต่ำกว่าเป้า §3.7 มาก — เหตุ/ตัวถือ HUMAN อยู่ใน §3.7 "ผลจริง" · md/csv ไม่มีถ้อยคำเพดาน 4 แล้ว (DPZ = VALUE-DRIFT)
   - **ถัง HUMAN → ถอดความ (Plan 4c-transcribe · 26 ก.ย. 69 · เจ้าของ: "ทำเหมือน ZTS … ห้ามวิเคราะห์ใหม่ งานคือย้ายจาก V2 => V3")** — ไม่มีการวิเคราะห์ใหม่/ข้อมูลใหม่ · worker แก้ draft ของ migrator ด้วยตัวเลข/ถ้อยคำของผู้เขียน v2 เท่านั้น (ตรรกะ `tools/migrate-v3/transcribe.js`):
     1. `node tools/migrate-v3.js draft <SYM> [--work-dir .work] [--force]` — migrateOne ทุกถัง → `.work/<SYM>.json` (คีย์ของ worker ตาม `S.OWNER` · คง `meta.migratedFrom` · ไม่มี `market`/`_sig`) + `.work/<SYM>.brief.md` (ถัง · เหตุผลทุกข้อ: H · TEXT LOST ต่อ zone · D/F · error ของ checkDoc บน draft · **ตัวเลขหลักของ v2 = เป้า**: FV (+ กรอบ) · ราคาเป้า Bear/Base/Bull + จำนวนปี · MOS ที่หน้า v2 พิมพ์ · คลาส verdict · สกุล · วันที่วิเคราะห์) · ไม่เขียนทับ draft เดิมถ้าไม่มี `--force` · `--work-dir` ใต้ `reports/` = ปฏิเสธ
     2. worker แก้ `.work/<SYM>.json` (ZTS pilot: solve `fvWeights` ให้ได้ FV ที่ผู้เขียนพิมพ์ · ถ่ายถ้อยคำเข้า note/desc · ฯลฯ)
     3. `node tools/migrate-v3.js adopt <SYM> [--doc …] [--today …]` — guard เดียวกับ `convert --write` (`MIGRATE_V3_ALLOW_REAL=1` บน `reports/` จริง · ต้องมีแถว manifest/`--head-manifest` · `meta.migratedFrom` คิดใหม่จากแถว manifest HEAD + freshHash ของหน้า v2 ปัจจุบันเสมอ: ไม่มี / `v2Hash` ค้างเพราะ cron ราคาแก้ `.html` หลัง draft = ใช้ค่าใหม่ · `updated` ไม่ตรงแถว manifest = ปฏิเสธ · ห้ามให้ worker ลบช่องนี้ — §8) → merge `market` ของหน้า v2 → (a) `checkDoc` 0 error ที่วันนี้ (b) ตัวเลขหลักตรง v2 **ภายในการปัดที่ผู้เขียนพิมพ์** (`NB.classifyNumber` · ฝั่ง v2 = literal ใน report-data ตามทศนิยมที่เขียน · MOS = ข้อความ `{{rd:mos}}` ที่ราคาเดียวกัน) · คลาส verdict เท่ากัน (c) symbol/currency เท่ากัน → `IO.write` + ลบ `.html` + checkDoc ซ้ำ (ตก = rollback แบบ `runConvert`) · equivalence gate (`EQ.compare` + ถัง) พิมพ์เป็น**ข้อมูลเท่านั้น** ไม่บล็อก · exit 0 / 1 (พิมพ์ทุกข้อที่ตกพร้อมค่า v2 · v3)
     4. controller: `npm run queue -- ship --migrate "<SYM> …" --model opus [--no-push]` (ไม่แก้ — `.json` มี `meta.migratedFrom` + `.html` ถูกลบ = แผนเดียวกับ convert)
     - **allowance ของใบ migrate เท่านั้น** (วัดบน draft ของ HUMAN 447 ใบ · ใบ NEW เท่าเดิม · มีเทสต์บวก/ลบทุกข้อใน `test/v3/migrate-transcribe.test.js`): ขา fv ขาเดียวที่ผู้เขียนประกาศ → **W33** แทน E17 (11 ใบ · §13 ข้อ 4 ยังค้างเป็นหนี้ชั้น 0) · declared sotp/nav ไม่มีตาราง → ใช้กติกา note (16/17 หน้า v2 ไม่มีตาราง) · `meta.sources` ≥ 2 (FISV — หน้า v2 ระบุ 2 แหล่ง) · `scenarios.cases[i].desc` ไม่มีได้ (DHR · EQIX Bull ไม่มีแถว "สถานการณ์" — render ไม่พิมพ์แถวว่าง)
...
| **Plan 4b** `migrate-v3.js` + sweep อ่านอย่างเดียว · **ไม่เขียน `reports/`** (dist byte-identical) | parser (locator เดิม) · leg classifier ตามตาราง §10.1 · **DCF N-stage** (§13-3) · weights/exit ใต้กฎ "ไม่ back-solve" (§10.1/§3.3) · แคตตาล็อกการ์ด + census fix + enum/nullable (§10 ช่องว่าง schema) · token ชุดที่เลือก (§10.1) · **กลไก `updated`** (§8 — เลือก + gate build 2 รอบ) · equivalence gate ต่อ zone (§10.2) + self-test `.ret` · รายงาน 3 ถัง + ตาราง VALUE-DRIFT เป็นไฟล์ใน docs · pre-patch อัตโนมัติของแถว v3 ใน preflight (`patchTargets` · `parseGateFailures` `.html` regex · ทาง revert · `ship --prepatch` รับ `.json` — open-items 66 · Plan 4a ให้แค่บรรทัดคำสั่งมือ) · `ship` commit `−.html +.json` ด้วยกัน (`ship.js:243` `existsSync` ทิ้ง .html ที่ลบ) · message `migrate: v3 <SYM…>` | sweep 909 ใบ → ตาราง 3 ถังสำหรับเจ้าของ (durable) · CLEAN มี TEXT LOST = 0 · self-test `.ret` ผ่าน · dist byte-identical · **ผล (25–26 ก.ย. 69)**: sweep ที่ commit (หลัง final-review fix round 2 · `1772355cd`) = CLEAN 11 / VALUE-DRIFT 426 / HUMAN 472 (ก่อน fix round 11 / 501 / 397 — gate ปิดจุดบอด I-1/I-2/I-3) · TEXT LOST ใน CLEAN 0 · self-test `.ret` ใน `test/v3/migrate-equiv.test.js` · DIST-PROOF base `6e7b6e3c0` vs head = DIST IDENTICAL (1069 ไฟล์ · OGE/ICC sha256 เท่าเดิม) · ซ้อม two-build gate + `ship --migrate --no-push` บน CCET/LEN ใน scratch ผ่าน · open-items 66 ปิด |
| **Plan 4c-prep** (roadmap 26 ก.ย. 69 · advisor) ปิดคลาส HUMAN เชิงโครงสร้างก่อนแบตช์ · **ไม่เขียน `reports/`** · ไม่ติด gate cron | ช่อง schema เพิ่ม (แบบ 6b): คำผู้เขียนใน gdots (65 ใบ) · vcell ที่ 3+ ของหมวด 8 (11) · legend annotation (20) · `.ret` annotation (89: อนุมาน `perYear` จากป้าย `/ปี` + `retNote` ที่เหลือ) · เพดาน custom card ผ่อนเฉพาะใบ migrate (102/28 ใบ · ต้องมี screenshot) · enum driver/exit ที่ถ้อยคำชัด (15/13) · `growth` ที่อ่านไม่ได้ (30) · tag แรกผิดรูป (14) · price-bound custom (18) · #67 aiModel ใน freshHash · #68 BDMS/TRMB · ปัด `pe`/`dividendYield` ใน reports.json ของ v3 ให้เท่า v2 (2 ตำแหน่ง) · **เครื่องมือแบตช์**: runner อ่านตารางที่อนุมัติ (symbol · bucket · driftClass) → `convert` ใหม่ตอนแบตช์ → **ปฏิเสธใบที่ bucket ไม่ตรงแถวที่อนุมัติ** (cron patch v2 ทุกวัน) → VALUE-DRIFT 1 หุ้น/commit (§13-1) → push ทีละ N commit (verify ของ pre-push รันครั้งเดียวต่อ push) | sweep ใหม่: HUMAN 472 → residual ที่ต้องใช้คนจริง · CLEAN ไม่ลด · DIST identical · คำตัดสินแทนคำถามเจ้าของ 5 ข้อบันทึกใน decisions พร้อมทางย้อนกลับ · **ผลจริง (Task 6)**: sweep `35b4be74e` CLEAN 12 · VALUE-DRIFT 450 · HUMAN 447 (เป้า ≈220–250 พลาด — §3.7) · DIST IDENTICAL (1069 ไฟล์ · OGE/ICC sha256 ตรง pin) · ซ้อม `batch` ใน scratch: CLEAN 12 ใบ = 1 commit `migrate: v3 …` (24 ไฟล์ reports) · verify ครั้งเดียว · `updated` ขยับ 0 · ตารางโกหก 1 แถว = ปฏิเสธ exit 3 ไม่เขียน · convert ซ้ำ = "เป็นใบ v3 แล้ว" exit 1 · screenshot `docs/superpowers/specs/assets/2026-09-26-4c-prep/` |
| **Plan 4c** แบตช์ migration — **gate: ห้าม merge แบตช์ก่อน cron ตามตาราง 3 รอบสะอาดที่ปิด #62 (26/27/28 ก.ย. 69)** | **ramp (advisor 26 ก.ย. 69)**: แบตช์แรก = ชุด CLEAN เท่านั้น → ดู cron 1 รอบที่ v3 lane ~10–20 ใบ → ชั้นปัดเศษ (fv-rounding/scn-tgt/index/multi-rounding) → mixed/prose-stale · advisor อนุมัติตารางก่อนทุก PR · แบตช์แรกต้องระบุใน PR body ว่าเป็นการลบ `.html` จริงครั้งแรกใน production (stat `−html +json` ตาม spec ไม่ใช่ "ลบรายงาน") · CLEAN commit ละ 50 (§13-1 · วัดได้ ~290 ⇒ ~6 commit) · VALUE-DRIFT 1 หุ้น/commit หลัง advisor อนุมัติตาราง (เจ้าของเห็นในรายงาน) · HUMAN = ถอดความ (draft → worker แก้ → adopt → ship --migrate · §10 ข้อ 4 · เจ้าของ 26 ก.ย. 69 "ห้ามวิเคราะห์ใหม่ … ย้าย V2 => V3") · screenshot ก่อน/หลัง 3 ใบ | `updated` + `reports.json.file` ไม่เปลี่ยนทุกใบ · DIST ต่างเฉพาะเซลล์ที่อนุมัติ (equivalence gate แทน byte identity — ต้องล้มเมื่อข้อความหาย ไม่ใช่แค่ตัวเลขต่าง) · stat ต่อหุ้น = `−<SYM>.html +<SYM>.json` (+ reports.json/seeds.json) |
| **Plan 4c-residual** ถอดความใบ HUMAN ที่เหลือ — `migrate-v3.js draft` → worker แก้ด้วยตัวเลข/ถ้อยคำของผู้เขียน v2 เท่านั้น → `adopt` (checkDoc 0 error + ตัวเลขหลักตรง v2) → `ship --migrate` (§10 ข้อ 4 · เจ้าของ 26 ก.ย. 69: "ห้ามวิเคราะห์ใหม่ … ย้าย V2 => V3" — ไม่มี prep/CROSS-VERIFY/ข้อมูลใหม่) | ไม่มีเวฟวิเคราะห์ใหม่ · เรียงตาม HUMAN reason | HUMAN = 0 ⇒ เข้า P7 |
...
3. **DCF**: สูตรมาตรฐาน 2-stage · ไม่ลง → `declared` · P6 วัดสัดส่วน DCF ที่ตกเป็น declared — **>10% ⇒ เพิ่ม 3-stage** ก่อน migrate ต่อ · **วัดแล้ว (Plan 4 Task 0 Q2 · 25 ก.ย. 69): ไม่ใช่ 2-stage โดยโครงสร้าง 88/304 = 28.9% > 10% ⇒ เกณฑ์ยิง → เพิ่ม N-stage schedule** (`stages:[{years,g}]` + terminal) ใน 4b พร้อมเทส — ครอบ phase ที่ 2 (37) · fade (24) · ลิสต์รายปี (26) → เหลือ declared 20 (6.6%) · revenue×margin (18) ยัง declared · 2-stage เดิม reproduce ได้ 98/304 · **ทำแล้ว (Plan 4b Task 2 + Task 4)**: `inputs.stages` 1–10 ช่วง (สคีมาบังคับ 2-stage หรือ stages ทางใดทางหนึ่ง) · parser ดึงได้ dcf 151 ขา (stages 49 · 2-stage 102 — วัดตอน Task 4 fix round 2) ที่เหลือ = `declared`
4. **E17 นับเฉพาะขา `role:'fv'`** (§3.6 I) — ขา context ไม่ช่วยให้ครบ ≥2 ขา · ใบที่เหลือขา fv 1 ขา = ละเมิดชั้น 0 → เข้าถัง **HUMAN** ตอน migrate (15 ใบ: EQIX APURE ARM CCJ CLS COHR CRWV DELTA ENTG EVR LEO MICRO MRNA POET RCAT) — *advisor อนุมัติเป็นคำตัดสินถาวร 24 ก.ย. 69 (เจ้าของมอบอำนาจอนุมัติให้ advisor)* · **แก้ 26 ก.ย. 69 ตามคำสั่งเจ้าของ ("ห้ามวิเคราะห์ใหม่ … ย้าย V2 => V3")**: ใบ migrate (`meta.migratedFrom`) ที่ผู้เขียน v2 ประกาศขา fv ขาเดียว — การถอดความที่ซื่อตรงคงขาเดียวนั้นไว้ และ gate แสดง **W33** (มองเห็น ไม่บล็อก · 0 ขา fv = E17 เสมอ) แทน E17 · ใบ NEW ไม่เปลี่ยน (E17 · EQIX-real คง E17)
