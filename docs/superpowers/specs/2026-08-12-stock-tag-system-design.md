...
| ขนาดคลัง | **80–120 slug** (เป้า — ดู §4.1 ถ้าเฟส 1 พบว่าไม่พอ) |
...
### 4.1 หลักประกันว่าคลังจะไม่บานกลับไปเป็น 2,084

จำนวน tag ไม่ได้ขึ้นกับความตั้งใจ แต่ขึ้นกับ **ใครมีสิทธิ์สร้าง slug ใหม่** — บังคับ 3 ชั้น:

1. **worker สร้าง slug เองไม่ได้** — เลือกจากคลังเท่านั้น · ต้องการธีมใหม่ → `--request` เข้าคิวรอเจ้าของ
2. **`tag-apply.js` ปฏิเสธ** slug นอกคลัง = ไม่เขียนไฟล์เลย (ทางเข้าเดียวที่แตะ `tags.json`)
3. **gate บล็อก push** — E40 error (slug นอกคลัง) · `tags-test` warn (tag มีสมาชิก <3)

ค่าเดิม 2,248 ค่าเป็น **วัตถุดิบให้จัดกลุ่ม ไม่ใช่ผลลัพธ์** — คลังโตได้เฉพาะตอนเจ้าของอนุมัติ

### 4.2 ผลสำรวจความเป็นไปได้ (วัดจากไฟล์จริง 908 ตัว)

ซอยป้ายเดิม (ตัด exchange) ตามตัวคั่น `• · / — |` แล้วนับคำที่ใช้ซ้ำ:

| เกณฑ์ | จำนวนคำ | ครอบหุ้นได้ |
|---|---|---|
| คำหลักไม่ซ้ำทั้งหมด | 1,450 | — |
| ใช้ซ้ำ ≥3 หุ้น | **151** | — |
| ใช้ซ้ำ ≥5 หุ้น | 71 | — |
| ใช้ซ้ำ ≥8 หุ้น | 34 | **503/908 (55%)** |
| ใช้ซ้ำ ≥12 หุ้น | 21 | 450/908 (50%) |

**ข้อสังเกตสำคัญ:** คำที่ใช้ซ้ำบ่อยที่สุดเป็นแกนที่ spec นี้ตัดทิ้ง — `large-cap` 63 · `technology` 50 · `healthcare` 49 · `industrials` 46 · `consumer staples` 44 (category + size) ที่เป็นธีมจริงมีเพียง `ai` 36 · `semiconductors` 33 · `cpo` 9

⇒ **แกนธีมอยู่ในหางยาว** เฟส 1 ต้องเป็นการจัดกลุ่มเชิงความหมายจริง (`"Data Center Cooling"` + `"AI Data Center • CPO Laser"` + `"Backup Power สำหรับ Data Center"` → `ai-datacenter` ทั้งที่ไม่มีคำร่วมกันเลย) ตัวเลข 151 จึงเป็น**พื้น ไม่ใช่เพดาน** และจุดที่จะตึงคือหุ้นราว 400 ตัวในหางที่ต้องยุบลงธีมกว้าง

**กติกาเมื่อ 120 ไม่พอ:** ถ้าเฟส 1 พบว่าการคุมให้อยู่ใน 120 ทำให้ต้องสร้าง tag กว้างจนกลายเป็น category → **เสนอตัวเลขใหม่พร้อมเหตุผลให้เจ้าของตัดสิน ห้ามยัดให้ลงกรอบ และห้ามปล่อยบานเอง** (บีบ = ได้ category ที่เจ้าของไม่เอา · ปล่อย = กลับไปเป็นปัญหา 2,084 เดิม)
...
| **W13** | warn | หุ้นไม่มีธีม `kind:"business"` เลย (มีแต่ธีม driver) |

> **แก้นิยาม W13 หลังติด tag ครบ 908 ตัว (13 ส.ค. 69):** เดิมออกแบบให้เตือนที่ "หุ้นมี tag เดียว" แต่ของจริงมีหุ้น tag เดียว **567 ตัว** ซึ่งเกือบทั้งหมดเป็นหุ้น US ที่ถูกต้องแล้ว — คลังธีม `driver` เป็นแกนไทยเป็นหลัก (`thai-consumption` · `thai-tourism` · `ai-datacenter`) หุ้น US ส่วนใหญ่จึงมีธีมธุรกิจที่ซื่อสัตย์ได้ตัวเดียว การเตือน 567 ครั้งคือเสียงรบกวนที่กลบสัญญาณจริง
> ⇒ เพิ่มฟิลด์ `kind` (`business`/`driver`) ให้ทุกธีมในคลัง แล้วเปลี่ยน W13 ให้เตือนสิ่งที่เป็นข้อบกพร่องจริง: **หุ้นที่มีแต่ธีม driver = บอกไม่ได้ว่าบริษัททำอะไร** ซึ่งเป็นช่องว่างของคลังที่ควรเข้าคิว `--request` · วัดจริงแล้วยิง 3 ครั้ง (CRWV, SISB, SPA) ตรงกับ vocab request ที่ worker ยื่นไว้พอดี
