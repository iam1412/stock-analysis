...
- **จำนวนหุ้นต่อรอบ (เวฟ) ไม่จำกัด** (ยกเลิก "เวฟละ ≤3" 12 ก.ค. 2569) — sequential + push รายตัว (ข้อ 4) ทำให้ blast radius = 1 หุ้นอยู่แล้ว **แต่จำกัดต่อ session: คิวยาวหั่นเป็น chunk ≤10 ตัว/session** — จบ chunk = ทุกตัว push แล้ว → **หยุด สรุปผล + วางคำสั่ง kickoff ให้ user เปิด session ใหม่ทำ chunk ถัดไป** (state ทั้งหมดอยู่ใน git + `price-flags.json` แล้ว ไม่มีอะไรต้อง handoff) · เหตุผล: cacheR/turn ของ controller โตตาม context — วัดจริง 12 ก.ค. 2569 รัน 25 ตัวรวดเดียว = 2.28M cacheR/หุ้น (~139k/turn) vs session สั้น 5 ตัว = 0.75M (~70k/turn)
...
- ฝั่ง controller: cacheR/turn **ไม่คงที่** — โตตาม context (~70k session สั้น → ~139k เมื่อรัน 25 ตัวรวด) → ต้องหั่น chunk ≤10 ตัว/session (ข้อ 3 + CLAUDE.md §4)
