...
> **v2 = รูปแบบที่ใช้จริงแล้วทั้งคลัง** (ย้ายเสร็จ 14 ก.ย. 2569 · ระยะ 2 ส่วน E · กวาดเพิ่มระยะ 3): **884/908 ใบเป็น v2** · เหลือ **24 ใบเป็น v1** (residue ที่ migrator ไม่ย้ายเพราะตัวเลขในใบขัดกันเอง — รายชื่อ/เหตุผลรายใบอยู่ใน `docs/superpowers/audit/2026-09-11-stock-analyzer/phase3-residue-disclosure.md` · ตัวเลขรอบระยะ 2 (865/43) อยู่ใน `phase2-exit.md` · แก้ต้นเหตุแล้วย้ายซ้ำต้องมีคนตัดสินค่าก่อน — ดู open-items #36) · **โหมด NEW ทุกใบใหม่เริ่มจาก skeleton v2** (ระยะ 2 ส่วน B) — ตัวอย่างข้างล่างนี้จึงเป็น v2 ตามที่ skeleton ใช้จริง · โหมด UPDATE บนไฟล์ v1 ที่เหลือ อ่านรูปแบบจากไฟล์จริงตรง ๆ (SKILL STEP 5B ข้อ 1) ไม่ต้องอิงตัวอย่างนี้
...
> ใหญ่ทั้งที่ตัวเลขเปลี่ยนแค่จุดเดียว — เรื่องนี้สำคัญเกินความสวยงาม: การย้ายคลัง 865 ไฟล์เป็น v2 (ส่วน E ของแผน · เสร็จ 14 ก.ย. 2569)
> รีวิวผ่าน diff แบบนี้เป๊ะ ๆ — ที่มาของค่าดู "ใครให้ค่าอะไร" ท้ายบล็อกนี้ ไม่ใช่คอมเมนต์ในตัว JSON
...
- `migrate-v3.js` (Plan 4b · spec §10) — **ย้ายใบ v2 → v3** (`reports/<SYM>.html` → `reports/<SYM>.json`): `sweep` = อ่านอย่างเดียวทั้งคลัง → parse → assemble → compute → render → equivalence gate ต่อ zone → **3 ถัง** CLEAN / VALUE-DRIFT (ต่างเฉพาะตัวเลขที่ v2 ค้าง/ปัด · csv มี `driftClass`/`maxDeltaPct`) / HUMAN (ข้อความหาย · ขาเป้านักวิเคราะห์เป็น fv · custom เกินเพดาน (ใบ migrate ≤8 · ใบ NEW ≤4) ฯลฯ) — ผลที่ commit: `docs/superpowers/specs/2026-09-25-v3-migration-sweep.{md,csv}` · `convert <SYM> [--write] [--accept-drift]` = เขียนทีละใบ (HUMAN ปฏิเสธ · VALUE-DRIFT ต้อง `--accept-drift` · ใส่ `reports/` จริงต้อง env `MIGRATE_V3_ALLOW_REAL=1`) → ประทับ `meta.migratedFrom {updated, v2Hash}` ให้ build คง `updated` ของ v2 · **แบตช์จริง = Plan 4c** ผ่าน `ship --migrate` (`docs/price-refresh.md`) · ช่อง schema ที่ migrator ใช้เก็บข้อความผู้เขียน (ไม่บังคับ · ใบ NEW ใช้ได้เหมือนกัน · render พิมพ์เมื่อมี): `text.chartHint` (ต่อท้ายป้ายหมวด 2) · `scenarios.hintNote` (ต่อท้าย hint หมวด 6) · `scenarios.cases[i].retNote` (ป้ายต่อท้ายผลตอบแทนต่อฉาก) · `multipleSource: 'author'` (ตัวคูณที่ผู้เขียนกำหนดเอง ไม่ใช่มัธยฐาน/peer/sector — ไม่ติดป้ายมัธยฐานให้ผิด) · `inputs.stages` (DCF N ช่วง) · `scenarios.exitDp` · tone `'none'` · `meta.headerTags` ≤3 (ใบ NEW ยังเขียน 0–2 — ช่องที่ 3 สำหรับ ADR/dual listing ของใบ migrate) · **Plan 4c-prep (spec §3.3/§3.7)** ช่องตัวเลือกใหม่: `meta.sectorLine` (≤100 · ข้อความ gdots) · `verdict.extraCells` (≤2 · vcell ที่ 3+ หมวด 8) · `text.legendNote` (≤80 · span ที่ 4 ของ legend หมวด 2) · `legs[i].inputs.base` (`eps`|`epsForward`|`epsFy` · pe) + `legs[i].baseLabel` (≤24) · `fundamentals.ffoBasis` `coreFfo` · driver `ebitdaPerShare` / exit `evebitda` (คู่กัน) · `metrics.custom` ≤8 เฉพาะใบ migrate (ใบ NEW ≤4) — ไม่มีค่า = หน้าเหมือนเดิมทุกไบต์ · **แบตช์ = `migrate-v3.js batch <ตาราง.csv> --class … --n … --model …`** (ปฏิเสธใบที่ถังต่างจากแถวที่อนุมัติ · รายละเอียด `docs/price-refresh.md` หัวข้อ "ใบ v3")
