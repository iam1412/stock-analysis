...
มี gate 2 ชั้น ต้องผ่านทั้งคู่ **ก่อน push เสมอ** (ดูข้อ 4; มี `pre-push` hook บังคับซ้ำ):

```bash
npm run verify           # ★ ครบชุด: check-reports → build → check-site
...
**ชั้น 1 — `test/check-reports.js`** (ตรวจ source `reports/<SYMBOL>.html` ทีละไฟล์ — 27 error + 9 warning):
- **โครงสร้าง:** DOCTYPE/`lang="th"`/ปิด `</html>`, `<title>` มีชื่อย่อ, `<h1>`, ครบ 8 section, กราฟ, gauge, เครื่องคิดเลข MOS, disclaimer, footer, header (ราคา+วันที่+แหล่งที่มา)
