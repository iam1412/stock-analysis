...
- **★ โมเดล (บังคับ · เพิ่ม 29 มิ.ย. 2569): ห้ามใช้ Haiku ทุกขั้น** — ห้ามทั้ง "Sonnet + Haiku" และ "Haiku ล้วน" (เคยทดสอบ Haiku เป็น worker แล้ว revert: gather อ่อน + agent ละเมิด protocol push เอง) · config ที่อนุญาตเท่านั้น: **all-Sonnet build + Opus(main) verify** (ค่าเริ่มต้น พิสูจน์แล้ว W5–W8) หรือ Opus+Sonnet · main session = Opus (verify/คุม), build worker = Sonnet (`model:"sonnet"`) · ดู memory `model-config-rules`
