...
3. **Export ไฟล์ผลลัพธ์เป็น `reports/<SYMBOL>.html`** เสมอ
   ...
   - **★ ปรับเนื้อหาตามเซกเตอร์ได้อิสระ** — gate บังคับแค่ "ครบ 8 section + ตัวเลขสอดคล้องกันเอง" **ไม่บังคับชุด metric/วิธี valuation** (โครงต้นแบบเป็น "ตัวอย่าง" ไม่ใช่แบบฟอร์มตายตัว):
     ...
     · **★ การเขียนค่าในเซลล์ P/E (กัน E21 ดักปี):** ห้ามขึ้นต้นด้วยปี เช่น `EPS FY2026 $8.44` — parser ของ E21 จะคว้า `2026` มาเป็น EPS · เขียนค่า `$` นำหน้าเสมอ → `EPS adj. $8.44 × P/E 20x`
   ...
   - **★ ใส่บล็อก `stock-meta`** (ตัวเลขสำหรับ **เรียง/คัดกรองหน้า index** — screener) — ถัดจาก meta ai-model ใน `<head>` ใส่
     ...
     · **★ `currency` = รหัส ISO 3 ตัว `"USD"`/`"THB"`** (E29 บังคับ `/^[A-Z]{3}$/`) — _คนละฟิลด์กับ_ `report-data.chart.currency` ที่เป็น **สัญลักษณ์** `$`/`฿` (§9) · **อย่าใส่ `"$"` ใน stock-meta** (push ไม่ผ่าน E29 ทันที — เคยพลาดทั้ง W29/W30)
...
- **★ โมเดล (บังคับ · 29 มิ.ย. 2569): ห้ามใช้ Haiku ทุกขั้น** — ห้ามทั้ง "Sonnet + Haiku" และ "Haiku ล้วน" (เคยทดสอบ Haiku เป็น worker แล้ว revert: gather อ่อน + agent ละเมิด protocol push เอง) · config ที่อนุญาต: Sonnet/Opus ผสมแบบใดก็ได้ — **ห้ามแค่ Haiku**
- **★ ค่าเริ่มต้น (user เลือก 30 มิ.ย. 2569 หลัง W31 กิน ~15% ของลิมิต 5 ชม. สำหรับ 3 หุ้น) = "All-Sonnet + escalate":** main controller = **Sonnet** + build worker = **Sonnet** (`model:"sonnet"`) — ถูกสุด, คุณภาพดี **ตราบใดที่ controller ตรวจข้ามแหล่ง price/EPS ≥2 + ดึงกราฟจริง + จับ split/ticker เองทุกตัว** (ตาข่ายนิรภัยอยู่ที่ controller ไม่ใช่ build) · **escalate หุ้นยาก (IPO <1 ปี / spinoff / split / cyclical ต้อง normalize EPS / ราคา cross-source ต่าง >5% หรือ EPS ไม่ตรง) 2 ระดับ:**
  - **(1) อัตโนมัติ ไม่ต้องบอก user** — spawn worker ตัวนั้นเป็น **Opus agent** (`Agent` `model:"opus"`) วิเคราะห์ลึกใน context แยก จ่าย Opus เฉพาะตัวนั้น ตัวอื่นยัง Sonnet (ครอบคลุมเกือบทุกเคส เพราะงานคือ "วิเคราะห์ลึก" ซึ่งเกิดใน worker)
  - **(2) นาน ๆ ครั้ง ต้องให้ user สลับ** — เฉพาะตอน **การตัดสิน publish/skip/delete ของ controller เองกำกวม** (เช่น HON ข้าม spinoff cliff) · controller สลับโมเดลตัวเองกลางทางไม่ได้ → **หยุด ping user** สลับ session เป็น Opus ชั่วคราว
  - token levers อื่น (fresh session/compact ทุก 1–2 เวฟ · ให้ agent ดึง WebFetch แล้วคืนเลขสั้น ๆ · targeted Read) ดู memory `model-config-rules`
...
- **★ STEP 0 ของทุก agent (กัน cwd-stray):** prompt ให้ agent เริ่มด้วย `cd <worktree path> && pwd` ยืนยันที่อยู่ + **ห้าม `cd` ลง main repo** (`/Users/somchai.s/Downloads/stock`) · บางครั้ง subagent หลงไปเขียน `reports/<SYM>.html` ใน main repo → `npm test` ของมันผ่าน (รันจากที่มันอยู่) gate เลยจับไม่ได้ แต่ไฟล์ **หายจาก worktree** · ตอน push เช็ค `ls reports/<SYM>.html` ใน worktree ก่อน — ถ้าไม่มี = ไปหยิบจาก main repo มา + ลบตัวหลง (ดู memory [[bulk-stock-analysis-workflow]])
...
- **ห้าม agent push เอง · push เป็น "ราย-เวฟ" (per-wave) ไม่ใช่รายตัว** — **รอ ≤3 agent ในเวฟเสร็จครบทุกตัวก่อน → main session รัน §4 (`verify → commit → pull --rebase → push`) ครั้งเดียวต่อเวฟ** (commit รวม `analyze: add A, B, C`)
  - ⚠️ **ทำไมไม่ push รายตัวทันทีที่เสร็จ:** `npm run verify` + pre-push hook สแกน **ทุกไฟล์** ใน `reports/` → ถ้า sibling ในเวฟเดียวกันยังเขียนค้าง (half-written) gate จะแดงและ **บล็อก push หุ้นที่เสร็จแล้ว** · per-stock จริงต้องมี per-symbol verify ที่ hook ยังไม่มี (ดู memory [[bulk-stock-analysis-workflow]])
- **main session push ทีละเวฟ (serialize)** — ห้าม push ซ้อนพร้อมกันหลายเวฟ/หลาย session กัน git race/conflict (1 เวฟ = 1 commit)
- ผลดี: context สะอาดสุด (1 หุ้น/agent ไม่มีปนเลข) · โหลดเป็นจังหวะ ≤3 ไม่ท่วม · เวฟเสร็จขึ้นเว็บเป็น checkpoint (เวฟหลังพลาดไม่ลบของที่ push แล้ว) · session อื่นเห็นเร็ว (หนุน §3.1)
...
git push origin HEAD:main          # 4. push → Cloudflare build & deploy เอง  ★ worktree ต้องใช้ HEAD:main (ไม่ใช่ 'main' เปล่า — local main ค้างหลัง HEAD จะโดน reject/รายงานไม่ขึ้น remote)
```
...
- ⏰ **Time Zone ของระบบ = Asia/Bangkok (UTC+7)** — ทุกการคิด "วันนี้"/ความสด (วันที่ใน header รายงาน · dedup 7 วัน · staleness 45/120 วัน) ใช้เวลาไทย · วันที่ในรายงานใช้ปี พ.ศ. (เช่น 2569)
- ❌ **ห้ามใช้ Haiku** ทุกขั้น (gather/build/verify) — ห้าม "Sonnet+Haiku" และ "Haiku ล้วน" · ค่าเริ่มต้น = **All-Sonnet main + escalate** ตัวยากเป็น Opus subagent อัตโนมัติ (ดู §3.2 + memory `model-config-rules`)
