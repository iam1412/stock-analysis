...
4. รัน `npm run verify` (check-reports → build → build-test → engine-exec → check-site) ให้ผ่านทั้งหมด
5. **Auto-push** (ดูข้อ 4)
...
- **spawn 1 Agent ต่อ ≤3 หุ้น — จัดกลุ่มตามเซกเตอร์** (ส่งหลาย tool call ในข้อความเดียวให้รันขนาน) — แต่ละ agent ทำ full analysis ตาม `/stock-analyzer` ของหุ้นในกลุ่มตัวเอง **รวมตรวจข้ามแหล่ง (ข้อ 2)** **เริ่มจากโครงต้นแบบ `_template/skeleton-{th,us}.html` (ข้อ 2.3)** เขียนผลลง **`reports/<SYMBOL>.html` เท่านั้น** (1 ไฟล์/1 หุ้น)
  - **ทำไม ≤3 ไม่ใช่ 1 หรือ 5:** 1 หุ้นกิน context ~40–80k (fetch ≥3 แหล่ง) → 3 ตัวยังสะอาด ไม่ชนเพดาน ~200k · **5 ตัวขึ้นไปเสี่ยง context ตัน + เลขปนข้ามหุ้น** (ตัวร้ายอันดับ 1 ของรีโปนี้ — งบหลายบริษัทใน context เดียว สลับ NIM/NPL/EPS ง่าย ขัดกับกฎ "เริ่มจาก skeleton อย่าก๊อปของเก่า") · ค่าโสหุ้ยที่ batch ช่วยลด **caching กลบไปเกือบหมด** (จ่ายเป็น cache-read ~10%) ไม่คุ้มเสี่ยง · **1 หุ้น/agent ก็ยังได้** ถ้าต้องการความปลอดภัยสูงสุด (≤3 คือเพดาน ไม่ใช่บังคับให้ครบ 3)
  - **จัดกลุ่มตามเซกเตอร์** (แบงก์อยู่ด้วยกัน · ค้าปลีกอยู่ด้วยกัน · อาหารอยู่ด้วยกัน) → research กลุ่ม/peer comp ใช้ร่วมในกลุ่มได้ = เปลี่ยนข้อเสีย batch เป็นข้อดี (แต่หุ้นคล้ายกันยิ่งต้องระวังสลับเลข)
  - **เสร็จหุ้นไหน เขียนไฟล์ `reports/<SYMBOL>.html` ทันที แล้ว "ลืม" ก่อนขึ้นตัวถัดไป** (ไฟล์รอดแม้ agent พังกลางคัน · context ไม่บวมข้ามตัว) · **ตรวจข้ามแหล่งเสร็จ → เก็บแค่ตัวเลข ทิ้งหน้าเว็บ** ไม่ให้ context บวม
- **ห้าม agent push เอง** (กฎเดิมยังอยู่) — แต่เปลี่ยนจาก "รวบ push ทีเดียวตอนจบ" เป็น: **agent คืนผล (ครบกลุ่ม หรือพังกลางคัน) → main session รัน §4 (`verify → commit → pull --rebase → push`) ของทุกไฟล์ที่ agent เขียนเสร็จ ทีละไฟล์ทันที ไม่รอ agent อื่น** (agent พังกลางกลุ่มก็เก็บไฟล์ที่เขียนเสร็จไป push ได้)
...
มี gate หลายชั้น ต้องผ่านทั้งหมด **ก่อน push เสมอ** (ดูข้อ 4; มี `pre-push` hook บังคับซ้ำ 5 ขั้น):

```bash
npm run verify           # ★ ครบชุด: check-reports → build → build-test → engine-exec → check-site
