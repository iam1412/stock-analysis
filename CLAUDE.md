...
- **★ โมเดล (บังคับ · 29 มิ.ย. 2569): ห้ามใช้ Haiku ทุกขั้น** — ห้ามทั้ง "Sonnet + Haiku" และ "Haiku ล้วน" (เคยทดสอบ Haiku เป็น worker แล้ว revert: gather อ่อน + agent ละเมิด protocol push เอง) · config ที่อนุญาต: Sonnet/Opus ผสมแบบใดก็ได้ — **ห้ามแค่ Haiku** · **ค่าเริ่มต้นปัจจุบัน (user เลือกหลัง W9) = all-Sonnet** (build worker = Sonnet `model:"sonnet"` + main/verify = Sonnet) — ถูกสุด, คุณภาพดี **ตราบใดที่ controller (main) ตรวจข้ามแหล่ง price/EPS ≥2 + ดึงกราฟจริง + จับ split/ticker เองทุกตัว** (ตาข่ายนิรภัยจริงอยู่ที่ controller ไม่ใช่ build) · ทางเลือก: main = Opus เมื่ออยากได้ judgment แข็งขึ้นบนหุ้นใหม่ยาก ๆ · ดู memory `model-config-rules`
...
- ❌ **ห้ามใช้ Haiku** ทุกขั้น (gather/build/verify) — ห้าม "Sonnet+Haiku" และ "Haiku ล้วน" · ใช้ all-Sonnet build + Opus verify (ดู §3.2 + memory `model-config-rules`)
