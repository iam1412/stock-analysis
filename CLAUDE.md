...
- **ใบ v2 (883/908 ใบ · ระยะ 3 กวาด residue เพิ่มจาก 865 · ดู open-items #36): cron แก้ `report-data.values` เท่านั้น** — ราคา/วันที่ราคา/MOS/`.chg`/verdict class/`#pxIn`/ป้าย gauge `#mCur` เป็น `{{rd:…}}` ที่ `build` render ให้ ⇒ cron **ไม่เขียนสำเนาใน HTML เลย** (7 ช่องนี้ต้องเป็น token เสมอ — pseudo-error `V2TOKENS` บังคับ · ยิง 0/908 · **ระยะ 3 Task 12 ขยายเป็น 13 ช่อง** โดยเพิ่มช่องที่ผูก **FV**: `.fv-box .r` · `legend` · `#mFair` · การ์ด "จุดซื้อ MOS 20/30%" · `vcell` "มูลค่าเหมาะสม" — เกณฑ์เดียวกัน คือ cron ไม่มีตัวเขียนให้ · `summary` ยังอยู่นอกรายการเพราะมี `summaryPlan` เขียนให้ — open-items #44) · การ์ดที่ derive จากราคา (P/E · Market Cap · P/S · ปันผล % · P/BV · % เป้า · หมวด 6 ที่ยัง literal) ยังผ่าน `patchDerived` เหมือน v1 แต่ทำบน view ที่ render แล้ว (`derivedPassV2` + keep-map — **token ชนะเสมอ** เมื่อชนกัน) · กระจก `stock-meta` เขียนแค่ `price/mos/upside/fairValue` (**`pe`/`dividendYield` เป็นของ pass derived เหมือน v1**) · เขียนกลับไม่ได้/วันที่ไม่ตรง = `patch-failed` เห็นในคิว ไม่เงียบ · 25 ใบที่ยังเป็น v1 เดินทางเดิมทุก byte → `docs/price-refresh.md` หัวข้อ "ใบ v2"
