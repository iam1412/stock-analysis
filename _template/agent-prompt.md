...
- **เขียน/แก้ไฟล์ตามโหมด:**
  - **NEW** = เตรียมเนื้อหาครบทุก STEP แล้ว **Write `reports/<SYM>.html` เต็มใบครั้งเดียว** ตามโครง skeleton (SKILL STEP 5A) — ห้าม `cp`+ไล่แทน token, ห้ามใช้ apply-edits/Edit เป็นชุด · หน้าตาบล็อก filled ทุกก้อน (report-data/vmethod/gauge/mos-verdict/ai-model/gdots/footer) → `docs/templates.md` §"ตัวอย่าง filled (NEW)" อ่านที่เดียวจบ ห้าม grep รายงานตัวอื่น · สีแบรนด์ = `node tools/pick-brand.js <SYM> "#hex"` **1 turn จบ** (ตรวจชน + ลง seeds.json + พิมพ์ theme/GDOTS ให้ copy) · เกณฑ์เลขสอดคล้อง (E21/E22 ±3% · โซน MOS W04 · W06) อ่านตาราง `docs/quality-gate.md` เท่านั้น — **ห้ามเปิด `test/check-reports.js` ทั้งก่อนและหลังเขียน**
