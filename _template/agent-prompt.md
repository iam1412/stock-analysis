...
- **เขียน/แก้ไฟล์ตามโหมด:**
  - **NEW** = เตรียมเนื้อหาครบทุก STEP แล้ว **Write `reports/<SYM>.html` เต็มใบครั้งเดียว** ตามโครง skeleton (SKILL STEP 5A) — ห้าม `cp`+ไล่แทน token, ห้ามใช้ apply-edits/Edit เป็นชุด · หน้าตาบล็อก filled + สูตรสีแบรนด์ → `docs/templates.md` §"ตัวอย่าง filled (NEW)" อ่านที่เดียวจบ
