...
async function main() {
  ...
  for (const f of files) {
    ...
    const d = decide({
      // v2: FV เจ้าของเดียว = report-data.fv (stock-meta.fairValue เป็นกระจก) · oldPrice = sm.price กระจกของ values.px ใช้ได้ทั้งสองทาง
      oldPrice: sm.price, newPrice: q.price, fv: fvOf(html, sm),
