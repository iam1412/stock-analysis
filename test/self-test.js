...
expect('E16', 'error', (h) => h.replace('<div class="big">−0.8%</div>', '<div class="big">+40%</div>'), 'MOS ไม่สอดคล้องกับ FV/ราคา');
...
expect('E19', 'error', (h) => h.replace('gpos(196.5)', 'gpos(300)'), 'gauge marker ปัจจุบันไม่ตรงราคา');
expect('E20', 'error', (h) => h.replace(/กรอบ\s*฿162\s*[–-]\s*฿225/, 'กรอบ ฿300 – ฿320'), 'Fair Value อยู่นอกกรอบ');
expect('W04', 'warn', (h) => h.replace('class="mos-verdict bad"', 'class="mos-verdict good"'), 'สี verdict (good) ขัดกับ MOS ต่ำ');
...
expect('E23', 'error', (h) => h.replace('value="196.5"', 'value="999"'), 'ราคา header ≠ ค่าตั้งต้นเครื่องคิดเลข');
...
expect('W06', 'warn', (h) => h.replace('MOS ~ −0.8% (เต็มมูลค่า)', 'MOS ~ −9% (เต็มมูลค่า)'), 'สรุประบุส่วนต่าง −9% แต่ MOS จริง ~ −0.8%');
expect('W07', 'warn', (h) => h.replace('class="v neu">~8.4x', 'class="v neu">~750x'), 'P/E ผิดวิสัย (750x)');
reject('W07', (h) => h.replace('class="v neu">~8.4x', 'class="v neu">~480x'), 'P/E ~480x (มัลติเพิลสูงจริงในตลาด AI เช่น ARM) → ไม่ใช่ค่าผิดวิสัย');
// W06: โซนกลาง — "เต็มมูลค่า" ขัดทิศเมื่อหุ้นถูกชัด (MOS +9%) แต่ไม่ขัดเมื่อ MOS ~0 (เคส MPWR)
expect('W06', 'warn', (h) => h.replace('class="r">฿195', 'class="r">฿180').replace('MOS ~ −0.8% (เต็มมูลค่า)', 'ถูกกว่ามูลค่า MOS ~ +8%'), 'หุ้นแพง (MOS −9%) แต่เขียน "ถูก/MOS+" → พลิกขั้ว');
reject('W06', (h) => h, 'MOS ~0% เขียน "เต็มมูลค่า" (เคส MPWR — ฐาน BBL ปัจจุบัน) → ไม่ฟ้องว่าขัดแย้ง');
...
expect('E29', 'error', (h) => h.replace('"price":196.5', '"price":"196.5"'), 'stock-meta.price เป็น string ไม่ใช่ตัวเลข');
expect('E30', 'error', (h) => h.replace('"price":196.5', '"price":999'), 'stock-meta.price ≠ ราคาที่โชว์ → ตรวจข้ามแหล่งในไฟล์');
expect('E31', 'error', (h) => h.replace('"upside":-0.8', '"upside":99'), 'stock-meta.upside ไม่สอดคล้องกับราคา&FV');
expect('W10', 'warn', (h) => h.replace('"pe":8.4', '"pe":50'), 'stock-meta.pe ≠ P/E ที่โชว์ (เตือน)');
// freshness — จำลอง "วันนี้" ผ่าน env STALE_TODAY (รายงานลงวันที่ราคา ~10 ก.ค. 2026)
{
...
{
  process.env.STALE_TODAY = '2026-09-10';
  const r = checkHtml(base, 'BBL.html');
...
// BBL ฐาน = "▲ +26.4% (รอบปี)" + theme เขียว (ขึ้น+เขียว = ถูก) + กราฟ 12 จุด ~1 ปี (ปลายกราฟ ~+26%)
expect('E34', 'error', (h) => h.replace('▲ +26.4% (รอบปี)', '▼ −31% ในรอบปี'), 'ป้าย change ขาลง (▼ −) แต่ theme ยังเขียว (เคส HMPRO/CPF) → ต้องจับ E34');
reject('E34', (h) => h.replace('▲ +26.4% (รอบปี)', '≈ ทรงตัว ในรอบปี'), 'ป้าย change "ทรงตัว" (ไม่มีทิศทาง) → ต้องไม่ฟ้อง E34');
// E35: header % ต้องเป็นผลตอบแทน "รอบปี" ไม่ใช่ % รายวัน/ช่วงอื่น (ยกเว้น IPO)
expect('E35', 'error', (h) => h.replace('▲ +26.4% (รอบปี)', '▲ +5.8% (22 มิ.ย.)'), 'ป้าย % รายวัน "(22 มิ.ย.)" (ไม่ใช่ "รอบปี") → ต้องจับ E35');
expect('E35', 'error', (h) => h.replace(/<div class="chg">[\s\S]*?<\/div>/i, '<div class="chg"></div>'), 'header ไม่มีป้าย % (.chg ว่าง) → ต้องจับ E35');
reject('E35', (h) => h.replace('▲ +26.4% (รอบปี)', '▲ +26.4% (ตั้งแต่ IPO)'), 'หุ้น IPO <1 ปี ใช้ "(ตั้งแต่ IPO)" → ต้องไม่ฟ้อง E35');
reject('E35', (h) => h, 'ป้าย "▲ +26.4% (รอบปี)" (รอบปี + ทิศทาง) → ต้องไม่ฟ้อง E35');
// E36: % รอบปี ต้องตรงกับผลตอบแทนปลายกราฟ (เดิม W11 → เลื่อนเป็น error)
expect('E36', 'error', (h) => h.replace('▲ +26.4% (รอบปี)', '▲ +99% ในรอบปี'), 'headline +99% รอบปี ขัดกับปลายกราฟ (~+26%) → ต้องจับ E36');
reject('E36', (h) => h.replace('▲ +26.4% (รอบปี)', '≈ ทรงตัว (รอบปี)'), 'ป้าย "ทรงตัว" (ไม่มี %) → ต้องไม่ฟ้อง E36');
// E37: กราฟต้อง ~1 ปี (ไม่เกิน ~13 จุด) — ขยายเป็น 14 จุดต้องโดนจับ
const extendChart = (h) => h.replace(/(<script[^>]*id="report-data"[^>]*>)([\s\S]*?)(<\/script>)/i, (m, a, json, b) => {
...
expect('E37', 'error', extendChart, 'กราฟ 14 จุด (>13 = เกิน ~1 ปีรายเดือน) → ต้องจับ E37');
reject('E37', (h) => h, 'กราฟฐาน BBL (12 จุด ~1 ปี) → ต้องไม่ฟ้อง E37');
expect('W12', 'warn', (h) => h.replace('["ส.ค.25", 155.5]', '["", 155.5]'), 'จุดกราฟแรก label ว่าง (["",155.5]) → เตือน W12');
...
reject('E29', (h) => h.replace('"dividendYield":6.1', '"dividendYield":null'), 'stock-meta: dividendYield = null (หุ้นไม่จ่ายปันผล) ยังถือว่าถูกต้อง');
reject('W10', (h) => h.replace('"dividendYield":6.1', '"dividendYield":null'), 'stock-meta: yield = null → ข้ามการเทียบ ไม่เตือน W10');
