...
// BBL ฐาน = "▲ +29.9% (รอบปี)" + theme เขียว (ขึ้น+เขียว = ถูก) + กราฟ 8 จุด ~1 ปี (ปลายกราฟ ~+30%)
expect('E34', 'error', (h) => h.replace('▲ +29.9% (รอบปี)', '▼ −31% ในรอบปี'), 'ป้าย change ขาลง (▼ −) แต่ theme ยังเขียว (เคส HMPRO/CPF) → ต้องจับ E34');
reject('E34', (h) => h.replace('▲ +29.9% (รอบปี)', '≈ ทรงตัว ในรอบปี'), 'ป้าย change "ทรงตัว" (ไม่มีทิศทาง) → ต้องไม่ฟ้อง E34');
// E35: header % ต้องเป็นผลตอบแทน "รอบปี" ไม่ใช่ % รายวัน/ช่วงอื่น (ยกเว้น IPO)
expect('E35', 'error', (h) => h.replace('▲ +29.9% (รอบปี)', '▲ +5.8% (22 มิ.ย.)'), 'ป้าย % รายวัน "(22 มิ.ย.)" (ไม่ใช่ "รอบปี") → ต้องจับ E35');
expect('E35', 'error', (h) => h.replace(/<div class="chg">[\s\S]*?<\/div>/i, '<div class="chg"></div>'), 'header ไม่มีป้าย % (.chg ว่าง) → ต้องจับ E35');
reject('E35', (h) => h.replace('▲ +29.9% (รอบปี)', '▲ +29.9% (ตั้งแต่ IPO)'), 'หุ้น IPO <1 ปี ใช้ "(ตั้งแต่ IPO)" → ต้องไม่ฟ้อง E35');
reject('E35', (h) => h, 'ป้าย "▲ +29.9% (รอบปี)" (รอบปี + ทิศทาง) → ต้องไม่ฟ้อง E35');
// E36: % รอบปี ต้องตรงกับผลตอบแทนปลายกราฟ (เดิม W11 → เลื่อนเป็น error)
expect('E36', 'error', (h) => h.replace('▲ +29.9% (รอบปี)', '▲ +99% ในรอบปี'), 'headline +99% รอบปี ขัดกับปลายกราฟ (~+30%) → ต้องจับ E36');
reject('E36', (h) => h.replace('▲ +29.9% (รอบปี)', '≈ ทรงตัว (รอบปี)'), 'ป้าย "ทรงตัว" (ไม่มี %) → ต้องไม่ฟ้อง E36');
...
reject('E37', (h) => h, 'กราฟฐาน BBL (8 จุด ~1 ปี) → ต้องไม่ฟ้อง E37');
expect('W12', 'warn', (h) => h.replace('["มิ.ย.25",137]', '["",137]'), 'จุดกราฟแรก label ว่าง (["",137]) → เตือน W12');
