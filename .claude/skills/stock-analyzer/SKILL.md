...
3. **Write `reports/<SYMBOL>.html` เต็มใบครั้งเดียว** ตามโครง skeleton ที่อ่านมา — แทนทุก `{{TOKEN}}` ด้วยข้อมูลจริง + ตัด comment "วิธีใช้" หัวไฟล์ออก:
  ...
  - **ช่อง "ส่วนต่างจากราคา" (vcell หมวด 8) เขียนได้รูปเดียว: `MOS ~ ±X%` เท่ากับ `.big`** — cron เขียนทับทั้งช่องทุกวัน (ระยะ 1 ข้อ D) · คำว่าถูก/แพง/เต็มมูลค่าอยู่ใน `.txt` ของกล่อง verdict
   ...
   - **★ E44 (error · ใบใหม่ทุกใบตั้งแต่ 14 ก.ย. 2569): ในย่อหน้า (`<p>` `<li>` `.txt` `.hint`) ห้ามพิมพ์ตัวเลขผูกราคาเป็นตัวเลข — ต้องเป็น token**
     ...
     **เลขที่ไม่ใช่ค่าของไฟล์ยังพิมพ์เป็นตัวเลขได้** (ราคาในอดีต · FV ของฉาก sensitivity · เป้าสูงสุด/ต่ำสุดของช่วง · เกณฑ์ "MOS 20%" ที่ไม่มีราคาต่อท้าย) — E44 ยิงเฉพาะเลขที่เท่ากับค่าที่ไฟล์ประกาศไว้ เว้นป้าย "ราคาปัจจุบัน/จุดเข้า/MOS ปัจจุบัน" ที่ห้ามเป็น literal เสมอ
     เผลอพิมพ์ไปแล้วและ **เลขตรงกับค่าปัจจุบันเป๊ะ** → `node tools/update-prices.js --heal-derived --write <SYM>` แปลงเป็น token ให้ (รูปแบบไม่ตรงทุกตัวอักษร เช่นเขียน `฿191` ขณะ token ให้ `฿191.00` = ต้องแก้เอง)
...
2. **แก้เฉพาะจุดที่เปลี่ยนจริง** (หลายจุด → รวมยิงใน Bash เดียวผ่าน `tools/apply-edits.js` แบบ STEP 5C ข้อ 3):
   ...
   - **★ E44 มีผลกับใบเก่าทันทีที่แตะ** (re-analysis ทุกโหมดต้องขยับ footer "ข้อมูล ณ" เป็นวันนี้ ⇒ ใบนั้นกลายเป็น "ใบใหม่" ในสายตา gate):
     ตัวเลขผูกราคาใน `<p>`/`<li>`/`.txt`/`.hint` ต้องกลายเป็น `{{rd:…}}` ทั้งหมด (รายการ token + ข้อยกเว้น อยู่ใน STEP 5A ข้อ 3)
     ทำครั้งเดียวตอนแก้ prose อยู่แล้ว: จุดที่เลขตรงค่าปัจจุบันเป๊ะ `node tools/update-prices.js --heal-derived --write <SYM>` แปลงให้ (คลังจริง ~30% ของจุด)
     ที่เหลือแทนเองในบล็อก `@@` เดียวกัน — **เลขที่ค้างอยู่แปลว่าประโยครอบ ๆ ก็อาจไม่จริงแล้ว ต้องอ่านแล้วเขียนใหม่ ไม่ใช่สลับแค่ตัวเลข**
