...
2. **เตรียมเนื้อหาให้ครบทุก STEP ก่อนเขียน** — ราคา/chart/ป้าย % จาก script (STEP 1) · FV/MOS (STEP 3) · scenario (STEP 4) · theme/การ์ดทุกใบ/prose 8 section — **หน้าตาบล็อก filled จริงทุกก้อน (report-data · การ์ด vmethod · gauge+scale · mos-verdict · เคส pe:null/ปันผล 0) + สูตรสีแบรนด์ 3 บรรทัด → `docs/templates.md` §"ตัวอย่าง filled (NEW)"** — อ่านที่เดียวจบ ห้าม grep รายงานตัวอื่น/ห้ามทดลอง `node -e` หา format เอง
...
- **สีแบรนด์** ใน `report-data.theme` เลือกตามลักษณะหุ้น ห้ามปล่อยน้ำเงิน default — เลือก hex ตาม `tools/brand-colors.md` → รัน `node tools/pick-brand.js <SYM> "#hex"` **ครั้งเดียวจบ** (ตรวจสีชน + ลง seeds.json + พิมพ์ theme 8 คีย์และบรรทัด `{{GDOTS}}` ให้ copy — ชน = script บอกรายชื่อ เลือกเฉดใหม่รันซ้ำ) · ห้ามอ่าน brandtheme.js / ห้าม `node -e` ทดลองเอง / ห้ามแก้ seeds.json มือ
