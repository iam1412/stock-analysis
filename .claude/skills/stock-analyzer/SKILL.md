...
- **เซลล์ P/E เขียนสัญลักษณ์สกุลเงินของใบ (`stock-meta.currency` — USD → `$` · THB → `฿`) นำหน้า EPS เสมอ** (`EPS adj. $8.44 × P/E 20x` / `EPS ฿2.10 × P/E 15x` — ขึ้นต้นด้วยปี parser จะคว้าปีเป็น EPS)
- ★ **การ์ด P/E ที่โชว์ตัวคูณ "ที่วัดได้"** (ราคาเฉลี่ยของปีงบที่ผ่านมา ÷ EPS ของปีนั้น) **ต้องมี `วัดได้` / `measured` / `ราคาเฉลี่ย ÷` ใน LABEL ของการ์ด** — ไม่งั้น healer ราคา (`patchDerived`) จะคำนวณใหม่ด้วยราคา spot แล้วทับเลขที่วัดจริง · การ์ดที่ label เป็น Forward / NTM / FY…E คือ spot ÷ EPS ล่วงหน้า อัปเดตตามราคาได้ถูกต้อง
