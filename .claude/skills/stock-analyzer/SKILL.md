...
เวลา = Asia/Bangkok (UTC+7) · วันที่ในรายงานใช้ปี พ.ศ. · ชื่อไฟล์ = `<SYMBOL>` พิมพ์ใหญ่เสมอ — NEW = `.json` ที่ `report.js save` เขียนให้ (worker ไม่ตั้งชื่อไฟล์เอง) · UPDATE = `.html` เดิม (override ชื่อ default อื่นทุกแบบ)
...
- **มาจากคิว price-flags** — triage ตามเหตุผลใน `price-flags.json`:
  ...
  - `drift-gt-*` (ตลาดขยับ ไม่ใช่ธุรกิจเปลี่ยน — flip ใน dead-band ±5 จุด กับราคาหลุดขอบ gauge cron patch เองแล้ว ไม่เข้าคิว ตั้งแต่ 2 ส.ค. 2569) → **UPDATE-LIGHT** (STEP 5C · ใบ v3 → STEP 5U) เมื่อไม่มีงบไตรมาส/ปีใหม่หลังวันที่ footer · มีงบใหม่/ไม่รู้วันงบ/ราคาขยับ >30% → **UPDATE เต็ม** (runbook ตัดสินให้ใน triage/prep — `--light-rule legacy` = กฎเดิม)
  - `suspect-split-or-data` → **UPDATE เต็ม** + ตรวจ split/ticker ก่อนเขียนเลขใด ๆ
  - `fetch-failed` / `patch-failed` → **อ่าน `detail` ของ flag ก่อนเสมอ** (cron patch ใบ v2 ได้แล้วตั้งแต่ระยะ 2 ส่วน D — `patch-failed` บนใบ v2 คือ tripwire ที่ตั้งใจให้ดัง **ห้ามปล่อยรอ**):
    - ใบ v2 (`report-data.v === 2`) + `detail` ขึ้นต้นด้วยข้อความของทาง v2 → **โครงไฟล์ใบนั้นต้องแก้** (ไม่ใช่ plumbing ไม่ใช่ราคา): `derivedPassV2: ลำดับ token…` (token `{{rd:…}}` งอก/หาย/สลับหลังประกอบกลับ) · `keepMap: ต่างกันเกิน…` (pass derived แก้ไฟล์เยอะผิดปกติ) · `derivedPassV2: ไม่มี override แต่ render(ผล) ≠ view…` (token ถูกวางผิดที่) · `derivedPassV2: วันที่ราคาที่โชว์ ≠ values.priceDate…` (วันที่ literal ในหัวรายงาน/วงเล็บทวน/.disc ที่ cron เขียนกลับไม่ได้ — แก้ข้อความรอบวันที่นั้นให้เป็น `{{rd:priceDate}}` หรือรูปที่ตัวอ่านรู้จัก) · `report-data.values.… ` / `ไม่มีบล็อก stock-meta` (สคีมา v2 เสีย) → แจ้ง controller พร้อม `detail` เต็ม · ตรวจซ้ำได้ offline ด้วย `node tools/update-prices.js --heal-derived <SYM>` (ใบที่ throw ขึ้น `⛔ <ไฟล์> <ข้อความ>` และ **exit code 1** — ใช้ยืนยันว่าแก้หายแล้ว)
    - `detail` อื่นทั้งหมด (ทั้งใบ v1 และ v2) → ปัญหา plumbing ตามเดิม (ticker เปลี่ยน/เพิกถอน/ประวัติกราฟ/`ไม่เจอ pattern: …` ของใบ v1) — **ไม่ใช่งานวิเคราะห์** แจ้ง controller ไปแก้ `tools/symbol-map.json` หรือเช็คเพิกถอน
  ...
  - `bad-chart` → **ซีรีส์กราฟจาก Yahoo ผสมสองฐาน** (split ที่ Yahoo ยังไม่ปรับย้อนหลังให้ครบ — เคส MNST 12 ส.ค. 2569 split 2:1: ก.ย.–ธ.ค. 25 ยังไม่ปรับ ปนกับ ม.ค. 26 ที่ปรับแล้ว) · อาการถ้าปล่อยผ่าน = กราฟมีหน้าผา −50% ปลอม + **ป้าย % รอบปีพลิกเครื่องหมาย** และ **gate จับไม่ได้** (E36 เทียบป้ายกับปลายกราฟ = ผิดพร้อมกันทั้งคู่จึงผ่าน) · `detail` ในคิวบอกจุดที่หลุดกรอบ 52 สัปดาห์ให้แล้ว — ทำตามลำดับ:
    ...
    3. **กราฟในไฟล์ยังผสมฐาน** → **UPDATE เต็ม** เหมือน `suspect-split-or-data`: คูณจุดก่อนวันมีผลด้วยตัวคูณ split ให้เป็นฐานเดียวกับราคาปัจจุบัน แล้วไล่แก้ EPS/FV/P-E/scenario ที่อิงราคาเก่าให้ครบ (ตัวเลขต่อหุ้นทั้งหมดเปลี่ยนฐาน ไม่ใช่แค่กราฟ)
    - ★ `node tools/update-prices.js --write --force <SYM>` ยัง**ใช้ได้ตามปกติ** ตอน bad-chart (ใบ v2 · ใบ v3 = controller ทำ ไม่ใช่ worker) — มันจะประทับราคา/วันที่/MOS ให้ แต่ **คงกราฟเดิมในไฟล์ไว้** (price-only) จึงไม่ลากฐานที่สองกลับเข้ามา · `fetch-facts.js` กับ `prep-stock.js` จะ **exit 2 หยุดให้เอง** ห้าม spawn worker ต่อจนกว่าจะแก้
...
- **ราคา + กราฟ ~1 ปี + ป้าย % รอบปี + สี** — ห้ามดึง Yahoo เอง / ห้ามคำนวณกราฟ-bounds เอง / ห้ามแต่งจุด:
  - NEW → **prompt มีบล็อก `=== FACTS ===` แล้ว = ห้ามรันซ้ำ ใช้เลย** (controller pre-fetch ผ่าน `prep-stock.js` มาแล้ว) · ไม่มีจึงรัน `node tools/fetch-facts.js <SYMBOL>` (หุ้นไทยเติม `--th` — ★ บังคับ กัน ticker ไทยชนหุ้น US เคส AIT/ORI) — ได้บล็อก chart+ป้าย+สี พร้อมวาง (= แหล่งราคาที่ 1) · **ใบ v3: บล็อก chart/ราคา/ป้าย % ไม่ต้องวางที่ไหน** — อยู่ใน sidecar `.queue/prep/<SYM>.json` แล้ว `report.js save` รวมเข้าใบให้เอง (บรรทัด "วางใน report-data" ในผลลัพธ์ script = ใบ v2)
  - UPDATE → ราคา/กราฟ/MOS ถูก patch มาแล้วโดย runbook (บล็อก "บันทึกจาก runbook" ใน prompt บอกไว้) — **รันซ้ำเฉพาะเมื่อบันทึกบอกว่ายังไม่ได้ patch**: `node tools/update-prices.js --write --force <SYMBOL>` (ปลอดภัยแล้ว — price-flags มี lock) · **ไม่มีบล็อก "บันทึกจาก runbook" ใน prompt = ยังไม่ได้ patch ⇒ รัน `node tools/update-prices.js --write --force <SYM>` ตามเดิม** — patch ราคา header/วันที่ราคา/กราฟ/ป้าย %/gauge.cur/MOS/pxIn/stock-meta ลงไฟล์เดิมให้เลย (= แหล่งราคาที่ 1) · ราคาหลุดขอบ gauge script ขยาย `gauge.min/max` ให้เอง — แต่ถ้า **FV เปลี่ยน** ต้องแก้โซน scale (MOS 20/30 = FV×0.8/0.7) เองใน STEP 5B ตามเดิม · **ใบ v3 (มี reports/<SYM>.json): ไม่รัน update-prices ไม่ว่ากรณีใด** — market.* เป็นของ cron/controller (บันทึกจาก runbook บอกว่าสดหรือยัง · ยังไม่สด = controller pre-patch ก่อน spawn) → ไปต่อ STEP 5U
...
- (v2) ธนาคาร → เน้น P/BV + Residual income · REIT → DDM/NAV (Occupancy/DPU) · หุ้นไม่ปันผล → ตัด DDM ใช้ P/E + DCF/Justified P/BV · หุ้นขาดทุน → ตัดการ์ด P/E + `stock-meta.pe/roe = null`
- (v2) gate เช็คคณิตเฉพาะวิธีชื่อ "P/E" (E21) และ "Justified P/BV" (E22) — ตั้งชื่อวิธีให้ตรงถ้าใช้
- (v2) **เซลล์ P/E เขียนสัญลักษณ์สกุลเงินของใบ (`stock-meta.currency` — USD → `$` · THB → `฿`) นำหน้า EPS เสมอ** (`EPS adj. $8.44 × P/E 20x` / `EPS ฿2.10 × P/E 15x` — ขึ้นต้นด้วยปี parser จะคว้าปีเป็น EPS)
- (v2) ★ **การ์ด P/E ที่โชว์ตัวคูณ "ที่วัดได้"** (ราคาเฉลี่ยของปีงบที่ผ่านมา ÷ EPS ของปีนั้น) **ต้องมี `วัดได้` / `measured` / `ราคาเฉลี่ย ÷` ใน LABEL ของการ์ด** — ไม่งั้น healer ราคา (`patchDerived`) จะคำนวณใหม่ด้วยราคา spot แล้วทับเลขที่วัดจริง · การ์ดที่ label เป็น Forward / NTM / FY…E คือ spot ÷ EPS ล่วงหน้า อัปเดตตามราคาได้ถูกต้อง
- **(v3)** สิ่งที่ตรงกันของ 4 ข้อข้างบน: หุ้นขาดทุน → ไม่ใส่ขา `pe`/การ์ด `pe` (schema ตัดสิน `stock-meta.pe` จาก `eps` เอง) · ชื่อวิธี = `method` ของขา (`pe`/`pbv`/`ddm`/…) ไม่ใช่ข้อความ · สกุลเงินมาจาก `currency` ของใบ ไม่ต้องพิมพ์นำหน้า EPS · การ์ด "วัดได้" = `multipleSource: 'median5y'/'median10y'` ใส่ `medianWindow` (ไม่มีป้ายให้พิมพ์)
- ★ **เฉลี่ย FV ต้องถ่วงตาม "ตระกูลสมมติฐาน" ไม่ใช่ตามจำนวนวิธี (กฎ 0.4c-bis)** — **DDM กับ Justified P/BV ใช้ (r,g) ชุดเดียวกัน = ตระกูลเดียว นับได้เสียงเดียว** ⇒ ถ้าใช้ 3 วิธีคือ P/E + DDM + Justified P/BV ให้คิดเป็น `FV = (P/E + ค่าเฉลี่ยของ DDM กับ JPBV)/2` **ไม่ใช่เฉลี่ยทั้งสามตรง ๆ** (ไม่งั้นสมมติฐาน r/g ชุดเดียวจะถ่วง 2 ต่อ 1 เหนือขาที่ยึดตลาด) · วิธีที่เหลือในตระกูลเดียวกันเขียนเป็น **"บริบท"** ในย่อหน้า full-disclosure · **ต่างกันเกิน 2.0 เท่า ⇒ ห้ามเฉลี่ยเลย** ใช้ขายึดตลาดเป็น headline (กฎ 0.4c) → `docs/quality-gate.md` §0.4c
- ★ **วัดได้จริงตระกูลเดียว (ไม่มีขายึดตลาดเลย) — อนุญาต (เจ้าของตัดสิน 22 ก.ย. 69, RULE-TENSION-001)**: ข้อยกเว้นของกฎ "ต้องมีอย่างน้อย 1 วิธีที่ไม่รับ (r,g) เป็น input" (`docs/quality-gate.md` §0.4) — E17 ยังบังคับ ≥2 การ์ด `.vmethod` เหมือนเดิม แต่ทำได้โดยใช้ **2 การ์ดตระกูลเดียวกัน** (เช่น DDM + Justified P/BV ทั้งคู่ (r,g) หรือ EV/Sales + P/S ชุดราคา/รายได้เดียวกัน) — เงื่อนไข **ต้องครบทั้ง 3 ข้อ**: (1) แต่ละการ์ดต้องเขียนกำกับชัดใน mdesc ว่า **"ตระกูลเดียวกัน นับเป็นหนึ่งเสียง"** (2) ต้องมีย่อหน้า full-disclosure สรุป FV/กรอบ + เหตุผลว่าทำไมไม่มีขายึดตลาด (3) **ต้องมีบรรทัดความไวเสมอ** (เงื่อนไขของการอนุญาต ไม่ใช่แค่บางกรณี): ตระกูล (r,g) → `FV ที่ r ± 0.5pp` (โดยเฉพาะเมื่อ `r − g` แคบกว่า ~5pp ที่ตัวคูณไวมาก) · ตระกูลตัวคูณ (P/S, EV/Sales ฯลฯ) → `FV ที่ตัวคูณ ± ช่วงประวัติที่วัดจริง` (เช่น เปอร์เซ็นไทล์ 25/75 ของมัธยฐานย้อนหลัง) — **★ ไม่ใช่ทางลัดเลี่ยงกฎ `|MOS| > 40% ต้องมีพยานที่ไม่ใช่ (r,g)` (§8 ชั้น 0)** ถ้า MOS เกิน 40% ยังต้องหาขาที่ไม่ใช่ (r,g)/ตระกูลเดียวมายืนยันเสมอ (ดูเคส GULF ที่แก้ด้วย SOTP แทนการใช้ข้อยกเว้นนี้) — เคสอ้างอิงที่ทำถูกแล้ว: `VLTO` (ตระกูล DCF — บรรทัดความไวสองด้าน กรณีฐาน/ระมัดระวัง) · `PDYN` (ตระกูลตัวคูณ — บรรทัดความไวสองด้าน ต่ำ/สูง) — **`COCOCO` ไม่ใช้เป็นแบบ**: บรรทัดความไวที่มีชี้ทางขึ้นอย่างเดียว (ROE/payout/g สูงขึ้น → FV สูงขึ้น) ไม่มีด้านลง จึงไม่ตรงเงื่อนไข (3) เต็มรูป — ต้องเติมด้านลงก่อนอ้างอิงเป็นแบบ
- ★★ **ตัวคูณเป้าหมายห้ามลอกมาจากตัวคูณปัจจุบัน (สมอตายวนกลับ · กฎ 0.4e · `W18`)** — "P/E เป้าหมาย ~36x เพราะใกล้เคียง P/E ปัจจุบัน 36x" ทำให้ขานั้น**คืนราคาตลาดกลับมาโดยโครงสร้าง** อ่านแล้วเหมือนมี valuation แต่บอกไม่ได้ว่าถูกหรือแพง (เคส ODFL 9 ก.ย. 69: ขาห่างราคา 0.1% · gate เดิมผ่าน 43/43) · เส้นแบ่งคือ **"ตัวตั้ง/ตัวคูณมาจากราคาวันนี้ไหม"** — ย้อนคำนวณ EBITDA จาก EV/EBITDA ปัจจุบัน หรือถอด EPS จาก ราคา ÷ fwd P/E ก็เข้าข่ายเดียวกัน
  ⇒ **ตัวคูณต้องมีสมอที่วัดจริง**: มัธยฐานย้อนหลังของหุ้นตัวเอง (controller ส่งมาให้ในบล็อก `=== ตัวคูณมัธยฐานย้อนหลัง: <SYM> ===` ใต้ FUNDAMENTALS — ถ้ามี **ห้ามประมาณเอง ห้ามรันซ้ำ**) หรือ peer ที่วัดจริง · อ้างตัวคูณปัจจุบันเป็น **บริบทเปรียบเทียบ** ได้ แต่ห้ามเป็นที่มาของเป้า
...
1. **`node tools/report.js init <SYM>`** (1 turn) — ได้ `.work/<SYM>.json` พร้อมช่อง `TODO` + บรรทัด ℹ/⚠: มัธยฐาน P/E + กรอบ (ใช้เป็น `legs[0].inputs.multiple`/`multipleRange`) · กับดัก vendor `[2c]` ฯลฯ · `sourceErrors` · exit ≠0 = ปฏิเสธ (1: มีใบอยู่แล้ว / มี draft `.work/<SYM>.json` ค้าง (ใช้ `--force` เฉพาะเมื่อตั้งใจทิ้ง draft) / ไม่มี sidecar · 2: คำสั่ง/symbol ผิดรูป) → รายงาน controller อย่าเดา
2. **อ่าน `.work/<SYM>.json` ครั้งเดียว แล้วเติมทุก `TODO` ใน Write/Edit ชุดเดียว** (1–2 turns) — กติกาที่ gate จับไม่ได้ทั้งหมดอยู่ตรงนี้:
   ...
   - ขา `declared` basis sotp/nav ต้องมี `inputs.extrasRef` ชี้ตาราง `extras[]` ที่มี `sumCol` (E52 ตรวจผลรวม ±1%) — รูปตารางดู `tools/v3/schema.js` (extras) · หุ้นขาดทุน (EPS ≤ 0): เติม `scenarios.driver`/`exitMetric` ตามค่าที่ TODO ระบุ (ห้ามใช้ eps/pe)
...
**ที่ต่างจากใบ v2 (STEP 5B/5C) ที่พลาดบ่อย**: ไม่มี footer "ข้อมูล ณ" ให้แก้ (มาจาก `meta.analysisDate`) · ไม่มี `{{rd:…}}`/E44 (token v3 คือ `{{px}}` `{{fv}}` … ตาม `show`) · ไม่มี `update-prices --write --force` ในงานของ worker · ไม่รัน `pick-brand` (สีอยู่ใน seeds.json แล้ว) · ไม่มี apply-edits
...
3. **Write `reports/<SYMBOL>.html` เต็มใบครั้งเดียว** ตามโครง skeleton ที่อ่านมา — แทนทุก `{{TOKEN}}` ด้วยข้อมูลจริง + ตัด comment "วิธีใช้" หัวไฟล์ออก:
  ...
  - **ช่อง "ส่วนต่างจากราคา" (vcell หมวด 8) เขียนได้รูปเดียว: `MOS ~ ±X%` เท่ากับ `.big`** — cron เขียนทับทั้งช่องทุกวัน (ระยะ 1 ข้อ D) · คำว่าถูก/แพง/เต็มมูลค่าอยู่ใน `.txt` ของกล่อง verdict
    **ไม่ต้องเลือกสีช่องนี้เอง** — skeleton ให้ `<div class="v">` เปล่า (ไม่มี `style`/class สี) cron จะเติม `class="v bad|ok|good"` ให้เองตาม `mosBand(mos)` ตอน patch รอบแรก (open-items #43) — ห้ามใส่ `style="color:#…"` มือ ไม่งั้นจะชนะ class ด้วย specificity แล้วค้างสีเดิมตลอดไป
   - **v2:** ตัวเลขผูกราคา/FV กรอก**ครั้งเดียว**ใน `report-data.values` (+`fv`) · ช่องที่เป็น `{{rd:…}}` ห้ามกรอก build render ให้ · `stock-meta` ยังเป็นกระจก กรอกให้ตรง values (E30)
   - **★ E44 (error · ใบใหม่ทุกใบตั้งแต่ 14 ก.ย. 2569): ในย่อหน้า (`<p>` `<li>` `.txt` `.hint`) ห้ามพิมพ์ตัวเลขผูกราคาเป็นตัวเลข — ต้องเป็น token**
     `{{rd:px}}` (ราคาปัจจุบัน/ล่าสุด/ตลาด/จุดเข้า) · `{{rd:fv}}` (มูลค่าเหมาะสม/Fair Value) · `{{rd:mos}}` (MOS ปัจจุบัน) · `{{rd:mos20}}`/`{{rd:mos30}}` (โซน MOS 20/30%) · `{{rd:analystTgt}} ({{rd:analystPct}})` (เป้าเฉลี่ยนักวิเคราะห์ + % จากราคา) · `{{rd:upside}}`
     เขียนได้ปกติ: ✅ `ราคาปัจจุบัน {{rd:px}} ต่ำกว่ามูลค่าเหมาะสม {{rd:fv}} ({{rd:mos}}) · รอโซน MOS 20% ที่ {{rd:mos20}}` — ❌ `ราคาปัจจุบัน ฿191 … มูลค่าเหมาะสม ฿195 (MOS ~2%) … MOS 20% (฿156)`
     - **ป้าย "เจ้าของ" ห้ามตามด้วยตัวเลขเด็ดขาด** (`ราคาปัจจุบัน/ล่าสุด/ตลาด/ปิด` · `จุดเข้า` · `MOS/ส่วนเผื่อ ~X%`) — **ไม่มีข้อยกเว้นให้ "ราคาในอดีต"**: E44 ฟ้องแม้เลขนั้นไม่ใช่ค่าของไฟล์ (และ token ก็ตอบแทนไม่ได้) ⇒ จะเล่าราคาในอดีตต้อง**เรียบเรียงประโยคใหม่ให้ไม่มีป้ายนั้น** เช่น `ราคาปิดวันที่ 5 ส.ค. 2569 อยู่ที่ $95.20` (ระบุวัน) ไม่ใช่ `ราคาปิด $95.20`
     - **ป้ายที่ชี้ได้หลายค่ายังพิมพ์ตัวเลขได้ ถ้าเลขนั้นไม่เท่าค่าฐานของไฟล์** (FV ของฉาก sensitivity · เป้าสูงสุด/ต่ำสุดของช่วง · โซน MOS ของ peer · เกณฑ์ "MOS 20%" ที่ไม่มีราคาต่อท้าย) — ⚠ **แต่ถ้ามันบังเอิญเท่ากับค่าฐานเป๊ะ E44 จะฟ้องและ healer จะแปลงเป็น token ให้** (แล้วเลขนั้นจะวิ่งตามค่าฐานตลอดไป) ⇒ เขียนเลขให้ต่าง หรือย้ายประโยคนั้นออกนอก `<p>`/`<li>`/`.txt`/`.hint`
     - **ขอบเขตที่ gate จับได้ ≠ ขอบเขตของกติกา**: ตัวจับต้องการให้ป้ายอยู่ชิดตัวเลข (~10–16 อักขระ ไม่ข้ามคำที่เปลี่ยนเจ้าของ) ⇒ ประโยคยาว (`ราคาปัจจุบันของหุ้นตัวนี้อยู่ที่ $188.00`) **หลุด gate แต่ยังผิดกติกา** — ต้องเขียนเป็น token เหมือนกัน
     - **★ footer "ข้อมูล ณ" ต้องเป็นวันที่เดียวที่ parse ได้** (`ข้อมูล ณ 14 ก.ย. 2569`) — **ห้ามเดือนล้วน** ("มิถุนายน 2569") **ห้ามช่วงข้ามเดือน** ("31 ก.ค. – 2 ส.ค. 2026" · ช่วงวันในเดือนเดียวกัน "22–23 มิ.ย. 2569" ใช้ได้) · เขียนผิดรูป = **W24 ฟ้อง และ E44 ข้ามทั้งใบ** (gate จะไม่บล็อก แต่ใบนั้นหลุดกติกา prose ไปทั้งใบ — คลังมีของจริงแบบนี้อยู่ 12 ใบ — 11 ใบเป็น v2 ที่ W24 ฟ้อง + MU ซึ่งเป็น v1 จึงอยู่นอกขอบเขต W24/E44 โดยดีไซน์)
     เผลอพิมพ์ไปแล้ว: **(1)** `node tools/update-prices.js --heal-derived <SYM>` (ไม่ใส่ `--write`) เพื่อดูว่าจุดไหนตรงทุก byte **(2)** แทนจุดที่เหลือเป็น token เอง **(3)** ค่อยปิดท้ายด้วย `--write` — ★ `--heal-derived --write` เขียนไฟล์ก็ต่อเมื่อ **gate ของไฟล์นั้นสะอาดหมดแล้ว** เหลือ E44 แม้จุดเดียว = ไม่เขียนอะไรเลย (คลังจริง: เคลียร์ได้ในรอบเดียว 27/550 ใบ)
...
## STEP 5B — เขียนรายงาน โหมด UPDATE (แก้เฉพาะจุด) (ใบ v3 → STEP 5U)
...
2. **แก้เฉพาะจุดที่เปลี่ยนจริง** (หลายจุด → รวมยิงใน Bash เดียวผ่าน `tools/apply-edits.js` แบบ STEP 5C ข้อ 3):
   - EPS / FV ทุกวิธี / จุดซื้อ MOS20-30 / scenario + `stock-meta` (fairValue, pe, eps, dividendYield, roe — **(v1) ยกเว้น price/mos/upside ที่ `update-prices.js --write --force` คำนวณให้อัตโนมัติตอนรันซ้ำ** — v2 ดูข้อถัดไป)
   - **v2:** แก้ตัวเลขผูกราคา = `node tools/apply-edits.js reports/<SYM>.html --set fv=<ใหม่> --set values.eps=<ใหม่> …` (ยิงเดี่ยว ๆ ไม่ต้องมี stdin) · **compose กับบล็อก `@@` (prose/การ์ดที่เป็น literal) ในคำสั่งเดียว → ต้องเติมแฟล็ก `--stdin` เสมอ**: `node tools/apply-edits.js reports/<SYM>.html --stdin --set fv=<ใหม่> <<'EOF' … EOF` (ลืม `--stdin` = สคริปต์ไม่อ่าน stdin เลย บล็อก `@@` หายเงียบ ไม่ error — ตั้งใจ กันเดาจังหวะแล้วค้าง/ดรอปข้อมูล) · หลังแก้ `fv` **ไม่ต้อง**รัน update-prices ซ้ำ (`{{rd:…}}` ในเนื้อหา render เอง) · **`stock-meta` บน v2 ซิงก์ให้เองในคำสั่งเดียวกัน**: หลัง `--set`/`--del` ใด ๆ บนใบ v2 สคริปต์รันกระจกตัวเดียวกับ cron (`price/mos/upside/fairValue` คำนวณจาก `values.px` + `fv`) ⇒ `--set fv=<ใหม่>` คำเดียวจบ ไม่ต้องคิด mos/upside เอง · **ห้าม `--set-meta` สี่คีย์นี้บน v2** (สคริปต์ปฏิเสธ — เป็นกระจก เขียนตรงแล้วไม่มีอะไรซิงก์กลับ) · `--set-meta` บน v2 ใช้ได้เฉพาะคีย์ที่ไม่ใช่กระจก: `roe` / `pe` / `dividendYield` (สองตัวหลังคือค่าที่การ์ด P/E / ปันผลที่เลือกโชว์ยืนอยู่ — cron ดูแลต่อจากฐานนั้น)
   - **prose ทุกประโยคที่อ้างเลขเก่า** (จุดเข้า / "แพง~X%" / เป้า / คำบรรยายกราฟ-ทิศทาง) + มุมมอง/catalyst ที่เปลี่ยน
   - **★ E44 มีผลกับใบเก่าทันทีที่แตะ** (re-analysis ทุกโหมดต้องขยับ footer "ข้อมูล ณ" เป็นวันนี้ ⇒ ใบนั้นกลายเป็น "ใบใหม่" ในสายตา gate):
     ตัวเลขผูกราคาใน `<p>`/`<li>`/`.txt`/`.hint` ต้องกลายเป็น `{{rd:…}}` ทั้งหมด (รายการ token + ข้อยกเว้น + ขอบเขตที่ gate จับได้ อยู่ใน STEP 5A ข้อ 3)
     **ลำดับที่ใช้ได้จริง — แก้มือก่อน แล้วค่อย heal ปิดท้าย** (สลับลำดับแล้วจะเสีย turn ฟรี):
     1. `node tools/update-prices.js --heal-derived <SYM>` **ไม่ใส่ `--write`** → ได้รายการจุดที่ literal ตรงกับ token ทุก byte (คลังจริงเฉลี่ย ~30% ของจุด)
     2. แทน**ทุกจุด**เป็น token เองในบล็อก `@@` เดียวกับที่แก้ prose อยู่แล้ว — **เลขที่ค้างอยู่แปลว่าประโยครอบ ๆ ก็อาจไม่จริงแล้ว ต้องอ่านแล้วเขียนใหม่ ไม่ใช่สลับแค่ตัวเลข**
     3. (ถ้าอยากให้เครื่องช่วยปิดท้าย) `--heal-derived --write <SYM>` — ★ ตัวมันเขียนไฟล์ **ก็ต่อเมื่อ gate ของไฟล์นั้นสะอาดหมดแล้ว** เหลือ E44 แม้จุดเดียว = ทิ้งงานที่ heal ไปทั้งไฟล์ ไม่เขียนอะไรเลย (คลังจริง: 27/550 ใบเท่านั้นที่จบในรอบเดียว) ⇒ **อย่านับว่า 30% ถูกแก้ไปแล้ว**
   ...
   - **รูป footer ต้อง parse ได้เป็นวันที่เดียว** (`ข้อมูล ณ 14 ก.ย. 2569`) — ห้ามเดือนล้วน/ช่วงข้ามเดือน ไม่งั้น W24 ฟ้องและ E44 ข้ามทั้งใบ (ดู STEP 5A ข้อ 3)
...
3. **v1: ถ้าแก้ `stock-meta.fairValue`** → รัน `node tools/update-prices.js --write --force <SYMBOL>` ซ้ำ (MOS/upside/ป้าย MOS คำนวณใหม่จาก FV ใหม่ให้เอง — ห้ามแก้เลขพวกนี้มือ) · **v2: ไม่ต้องรันซ้ำ** — ดูข้อ 2 (`--set fv=…` คำเดียว stock-meta ซิงก์ fairValue/mos/upside ให้เอง · `--set-meta` เฉพาะ roe/pe/dividendYield)
...
> **ใบ v3 → STEP 5U (LIGHT) ไม่ใช่หัวข้อนี้**
...
2. **จุดตัดสิน (กฎใหม่ 22 ก.ย. 69)**: **LIGHT ⇔ ไม่มีงบไตรมาส/ปีที่ประกาศหลังวันที่ footer "ข้อมูล ณ" เดิม** — runbook (`preflight`/`prep`) ตัดสินโหมดให้แล้ว ถ้าหัว prompt บอก UPDATE-LIGHT = FV เดิมยืน ไปข้อ 3 · **FULL** ถ้ามีงบใหม่ · วันงบไม่ทราบ · ราคาขยับ >30% จากวันก่อน · split · ปันผลถูกตัด · ("งบ" = รายไตรมาส/ปีเท่านั้น 8-K/guidance/M&A ไม่นับ — ช่องโหว่ที่รู้แล้ว ⇒ เจอสัญญาณเหล่านี้เองระหว่างทำ **ยกระดับเป็น UPDATE เต็ม** STEP 2→5B)
   - EPS(TTM) vendor ต่างจากใบ >2% = **คำเตือนใน prep เท่านั้น ไม่เปลี่ยนโหมดแล้ว** — prep พิมพ์สองฐาน (EPS ฐานของใบ vs vendor GAAP TTM) ให้ดูว่าต่างเพราะฐานคนละแบบ (adj vs GAAP) หรือเพราะงวดใหม่จริง
   - **UPDATE-LIGHT ยัง "ต้องแก้ไฟล์" เสมอ**: ประทับวันที่ footer ("ข้อมูล ณ") และวันที่ใน disclaimer เป็นวันนี้ · ถ้า gate E44 ยิง (ใบ v2 ที่ footer ผ่าน 2026-09-14) ให้แปลงเลขผูกราคาในย่อหน้าเป็น token `{{rd:…}}` · run LIGHT ที่ไม่แก้อะไรเลย = ไม่ถูกต้อง (ใช้ไม่ได้)
3. **แก้ไฟล์ — 2 turns เท่านั้น**: turn แรก `grep -n` หา**ทุกจุด**ที่อ้างเลขเก่าในไฟล์ (จุดเข้า / "แพง~X%" / คำบรรยายทิศกราฟ / gauge ถ้า script เตือนหลุดช่วง / วันที่ footer / `meta ai-model`) **+ `sed -n 'X,Yp'` ดึงบรรทัดจริงของทุกจุดใน Bash เดียวกัน** → turn ถัดไป apply ทุกจุดใน **Bash call เดียว** ผ่าน `tools/apply-edits.js` (all-or-nothing — จุดไหนหาไม่เจอ/ไม่ unique = ไม่เขียนไฟล์เลย script บอกทุกจุดที่พัง แก้ block แล้วรันใหม่):
   ...
   ⚠ **E44 (ใบ v2 ที่ขยับ footer เป็นวันนี้)**: ตัวเลขผูกราคาในย่อหน้าต้องเป็น `{{rd:…}}` — รวมไว้ใน `grep -n` รอบแรกและบล็อก `@@` รอบเดียวกันนี้เลย (รายละเอียด/รายการ token: STEP 5B ข้อ 2 · STEP 5A ข้อ 3) · อย่าปล่อยให้ไปโผล่ตอน `npm test` แล้วเสียอีก 2 turns
