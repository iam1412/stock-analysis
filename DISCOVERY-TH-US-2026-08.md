...
## ✅ สถานะความคืบหน้า — **25 / 129** (อัปเดต 8 ส.ค. 2569)

> เป้าจริง = **129 ตัว** (130 − TECK ที่ตัดออก) · ทุกตัวที่ทำแล้ว **push ขึ้น `main` เรียบร้อย**

**เสร็จแล้ว 25 ตัว** (เรียงตามลำดับที่ทำ):
...
**ตัวถัดไปที่ prep เสร็จแล้ว (สีแบรนด์อยู่ใน `tools/seeds.json` แล้ว):** `MFC` · `SLF` (1A ที่เหลือ) · `DB` · `ESLT` (1B ที่เหลือ) → จบ Tier 1 แล้วต่อ Tier 2 (`FERG` `WSO` `CSL` `MANH` `BWXT` `CACI` `TW` `OWL` `LPLA` `EWBC` `RYAN` `SSNC` …)
